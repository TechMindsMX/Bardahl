...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	public function save ($pFilename = null) {
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new PHPExcel_Writer_Excel5_Worksheet($this->_str_total,
																				$this->_str_unique,
																				$this->_str_table,
																				$this->_colors,
																				$this->_parser,
		...
		for ($i = 0; $i < 15; ++$i) {
			$this->_writerWorkbook->addXfWriter ($cellXfCollection[0],
												 true);
		}

		// add all the cell Xfs
		foreach ($this->_phpExcel->getCellXfCollection () as $style) {
			$this->_writerWorkbook->addXfWriter ($style,
												 false);
		}
		...
		$root = new PHPExcel_Shared_OLE_PPS_Root(time (),
												 time (),
												 $arrRootData);
		// save the OLE file

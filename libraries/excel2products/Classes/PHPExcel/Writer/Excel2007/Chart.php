...
class PHPExcel_Writer_Excel2007_Chart extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	public function writeChart (PHPExcel_Chart $pChart = null) {
		...
		$this->_writeTitle ($pChart->getTitle (),
							$objWriter);
		...
		$this->_writeLegend ($pChart->getLegend (),
							 $objWriter);
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeTitle (PHPExcel_Chart_Title $title = null,
								  $objWriter) {
		if (is_null ($title)) {
			return;
		}
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeLayout (PHPExcel_Chart_Layout $layout = NULL,
								   $objWriter) {
		...
		foreach ($chartTypes as $chartType) {
			...
			for ($i = 0; $i < $groupCount; ++$i) {
				...
				if ($groupType == $chartType) {
					...
					$this->_writePlotGroup ($plotGroup,
											$chartType,
											$objWriter,
...
//					} while ($id1 == $id2);
	...
	private function _writePlotGroup ($plotGroup,
		...
		foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			...
			if ($plotSeriesLabel && ($plotSeriesLabel->getPointCount () > 0)) {
				...
				$this->_writePlotSeriesLabel ($plotSeriesLabel,
											  $objWriter);
				$objWriter->endElement ();
				$objWriter->endElement ();
			}
			...
			if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount () > 0)) {
				...
				$this->_writePlotSeriesValues ($plotSeriesCategory,
											   $objWriter,
											   $groupType,
											   'str',
											   $pSheet);
			...
			if ($plotSeriesValues) {
				...
				$this->_writePlotSeriesValues ($plotSeriesValues,
											   $objWriter,
											   $groupType,
											   'num',
											   $pSheet);
			...
			if ($groupType === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
				$this->_writeBubbles ($plotSeriesValues,
									  $objWriter,
									  $pSheet);
			}
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writePlotSeriesLabel ($plotSeriesLabel,
											$objWriter) {
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeBubbles ($plotSeriesValues,
									$objWriter,
									PHPExcel_Worksheet $pSheet) {
		if (is_null ($plotSeriesValues)) {
			return;
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeCatAx ($objWriter,
								  PHPExcel_Chart_PlotArea $plotArea,
								  $xAxisLabel,
								  $groupType,
								  $id1,
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeLegend (PHPExcel_Chart_Legend $legend = null,
								   $objWriter) {
		if (is_null ($legend)) {
			return;
		}

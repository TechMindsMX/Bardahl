...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
	 ...
	 */
	public function __construct (&$str_total,
								 &$str_unique,
								 &$str_table,
								 &$colors,
								 $parser,
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
			...
			if ($cVal instanceof PHPExcel_RichText) {
				...
				$arrcRun = array ();
				$str_len = PHPExcel_Shared_String::CountCharacters ($cVal->getPlainText (),
																	'UTF-8');
				$str_pos = 0;
				...
				switch ($cell->getDatatype ()) {
					...
					case PHPExcel_Cell_DataType::TYPE_NULL:
						if ($cVal === '' || $cVal === null) {
							$this->_writeBlank ($row,
												$column,
												$xfIndex);
						} else {
							$this->_writeString ($row,
												 $column,
												 $cVal,
												 $xfIndex);
						}
		...
		foreach ($_phpSheet->getHyperLinkCollection () as $coordinate => $hyperlink) {
			...
			if (strpos ($url,
				...
				if (preg_match ('/^(http:|https:|ftp:|mailto:)/',
								$url)) {
					// URL
					// $url = $url;
	...
	private function _writeGuts () {
		...
		$colcount = count ($this->_colinfo);
		for ($i = 0; $i < $colcount; ++$i) {
			$col_level = max ($this->_colinfo[$i][5],
							  $col_level);
		}
	 ...
	 */
	private function _writeRow ($row,
								$height,
								$xfIndex,
								$hidden = false,
								$level = 0) {
	 ...
	 */
	private function _writeRichTextString ($row,
										   $col,
										   $str,
										   $xfIndex,
										   $arrcRun) {
	 ...
	 */
	private function _writeString ($row,
								   $col,
								   $str,
								   $xfIndex) {
		$this->_writeLabelSst ($row,
							   $col,
							   $str,
							   $xfIndex);
	}
	 ...
	 * @return integer
	 */
	private function _writeLabelSst ($row,
									 $col,
									 $str,
									 $xfIndex) {
		$record = 0x00FD;                   // Record identifier
	 ...
	 * @return integer
	 */
	private function _writeNumber ($row,
								   $col,
								   $num,
								   $xfIndex) {
	 ...
	 * @return integer
	 */
	private function _writeFormula ($row,
									$col,
									$formula,
									$xfIndex,
									$calculatedValue) {
			...
			if (is_bool ($calculatedValue)) {
				...
				if (array_key_exists ($calculatedValue,
					...
					$num = pack ('CCCvCv',
								 0x02,
								 0x00,
								 self::_mapErrorCode ($calculatedValue),
								 0x00,
								 0x00,
								 0xFFFF);
	 ...
	 * @param int $xfIndex
	 */
	private function _writeBoolErr ($row,
									$col,
									$value,
									$isError,
									$xfIndex) {
	...
	private function _writeZoom () {
		...
		$header = pack ("vv",
						...
						$length);
		$data = pack ("vv",
					  $this->_phpSheet->getSheetView ()->getZoomScale (),
					  100);
		$this->_append ($header . $data);
	}
	 ...
	 * @return integer
	 */
	private function _writeUrl ($row,
								$col,
								$url) {
		// Add start row and col to arg list
		return ($this->_writeUrlRange ($row,
									   $col,
									   $row,
									   $col,
									   $url));
	...
	function _writeUrlRange ($row1,
		...
		if (preg_match ('[^internal:]',
						$url)) {
			return ($this->_writeUrlInternal ($row1,
											  $col1,
											  $row2,
											  $col2,
											  $url));
		...
		if (preg_match ('[^external:]',
						$url)) {
			return ($this->_writeUrlExternal ($row1,
											  $col1,
											  $row2,
											  $col2,
											  $url));
		}
		return ($this->_writeUrlWeb ($row1,
									 $col1,
									 $row2,
									 $col2,
									 $url));
	 ...
	 * @return integer
	 */
	function _writeUrlInternal ($row1,
								$col1,
								$row2,
								$col2,
								$url) {
		...
		$url = PHPExcel_Shared_String::ConvertEncoding ($url,
														'UTF-16LE',
														'UTF-8');
	 ...
	 * @return integer
	 */
	function _writeUrlExternal ($row1,
								$col1,
								$row2,
								$col2,
								$url) {
		...
		$dir_short_len = pack ("V",
							   strlen ($dir_short));
		$dir_long_len = pack ("V",
							  strlen ($dir_long));
		$stream_len = pack ("V",
							0);//strlen($dir_long) + 0x06);
	...
	private function _writeCFRule (PHPExcel_Style_Conditional $conditional) {
		...
		if ($bFormatFill == 1) {
			...
			$dataBlockFill = pack ('v',
								   $blockFillPatternStyle);
			$dataBlockFill .= pack ('v',
									$colorIdxFg | ($colorIdxBg << 7));
		}
		if ($bFormatProt == 1) {
	 ...
	 */
	function setOutline ($visible = true,
						 $symbols_below = true,
						 $symbols_right = true,
						 $auto_style = false) {
	 ...
	 */
	function _positionImage ($col_start,
							 $row_start,
							 $x1,
							 $y1,
							 $width,
		...
		if (PHPExcel_Shared_Excel5::sizeRow ($this->_phpSheet,
											 $row_start + 1) == 0
		) {
			return;
		}
		...
		$this->_writeObjPicture ($col_start,
								 $x1,
								 $row_start,
								 $y1,
								 $col_end,
	 ...
	 */
	private function _writeObjPicture ($colL,
									   $dxL,
									   $rwT,
									   $dyT,
									   $colR,
	 ...
	 * @return integer
	 */
	private function _writeLabel ($row,
								  $col,
								  $str,
								  $xfIndex) {
		$strlen = strlen ($str);
	 ...
	 */
	private function _writeNote ($row,
								 $col,
								 $note) {
		...
		$data = pack ("vvv",
					  ...
					  $col,
					  $note_length);
		$this->_append ($header . $data . substr ($note,
												  0,
												  2048));

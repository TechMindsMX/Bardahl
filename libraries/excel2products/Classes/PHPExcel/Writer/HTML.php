...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	public function save ($pFilename = null) {
		...
		// Write headers
		fwrite ($fileHandle,
				$this->generateHTMLHeader (!$this->_useInlineCss));
	...
	private function _calculateSpans () {
		...
		$sheetIndexes = $this->_sheetIndex !== null ?
			array ($this->_sheetIndex) : range (0,
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	public function generateSheetData () {
		...
		foreach ($sheets as $sheet) {
			...
			$html .= $this->_extendRowsForChartsAndImages ($sheet,
														   $row);
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _generateRow (PHPExcel_Worksheet $pSheet,
								   $pValues = null,
								   $pRow = 0) {
		if (is_array ($pValues)) {
			// Construct HTML
			...
			foreach ($pValues as $cellAddress) {
				...
				if ($writeCell) {
					...
					// Image?
					$html .= $this->_writeImageInCell ($pSheet,
													   $coordinate);

					// Chart?
					if ($this->_includeCharts) {
						$html .= $this->_writeChartInCell ($pSheet,
														   $coordinate);
					}
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeImageInCell (PHPExcel_Worksheet $pSheet,
										$coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeChartInCell (PHPExcel_Worksheet $pSheet,
										$coordinates) {
		// Construct HTML
		$html = '';
		...
		foreach ($pSheet->getChartCollection () as $chart) {
			if ($chart instanceof PHPExcel_Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					...
					if ($fp = fopen ($chartFileName,
									 "rb",
									 0)
					) {
						$picture = fread ($fp,
										  filesize ($chartFileName));
						fclose ($fp);
						// base64 encode the binary data, then break it
						// into chunks according to RFC 2045 semantics
						$base64 = chunk_split (base64_encode ($picture));
	 ...
	 * @return string
	 */
	public function formatColor ($pValue,
								 $pFormat) {

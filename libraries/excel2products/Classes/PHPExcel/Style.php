		...
		if (is_array ($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if ($pAdvanced && isset($pStyles['borders'])) {
					...
					$xMax = min ($rangeEnd[0] - $rangeStart[0] + 1,
								 3);
					$yMax = min ($rangeEnd[1] - $rangeStart[1] + 1,
								 3);
					...
					for ($x = 1; $x <= $xMax; ++$x) {
						...
						for ($y = 1; $y <= $yMax; ++$y) {
							...
							$innerEdges = array_diff (array ('top', 'right', 'bottom', 'left'),
													  $edges);
				...
				if (preg_match ('/^[A-Z]+1:[A-Z]+1048576$/',
								$pRange)) {
					$selectionType = 'COLUMN';
				} else {
					if (preg_match ('/^A[0-9]+:XFD[0-9]+$/',
									$pRange)) {
						$selectionType = 'ROW';
					} else {
						$selectionType = 'CELL';
				...
				switch ($selectionType) {
					...
					case 'CELL':
						for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
							for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
								$cell = $this->getActiveSheet ()->getCellByColumnAndRow ($col,
																						 $row);
				...
				if (array_key_exists ('borders',
									  $pStyles)) {
					$this->getBorders ()->applyFromArray ($pStyles['borders']);
				}
				if (array_key_exists ('alignment',
									  $pStyles)) {
					$this->getAlignment ()->applyFromArray ($pStyles['alignment']);
				}
				if (array_key_exists ('numberformat',
									  $pStyles)) {
					$this->getNumberFormat ()->applyFromArray ($pStyles['numberformat']);
				}
				if (array_key_exists ('protection',
									  $pStyles)) {
					$this->getProtection ()->applyFromArray ($pStyles['protection']);
				}
				if (array_key_exists ('quotePrefix',
									  $pStyles)) {

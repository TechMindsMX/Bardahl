...
//		}
	...
	public function _getFromZipArchive ($archive,
										$fileName = '') {
		// Root-relative paths
		if (strpos ($fileName,
					'//') !== false
		...
		if ($contents === false) {
			$contents = $archive->getFromName (substr ($fileName,
													   1));
		}
	...
	public function listWorksheetInfo ($pFilename) {
		...
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				...
				$xmlWorkbook = simplexml_load_string ($this->_getFromZipArchive ($zip,
																				 "{$rel['Target']}"),
													  'SimpleXMLElement',
	...
	private static function array_item ($array,
										$key = 0) {
	...
	public function load ($pFilename) {
		...
		foreach ($wbRels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
					...
					$xmlTheme = simplexml_load_string ($this->_getFromZipArchive ($zip,
																				  "xl/{$rel['Target']}"),
													   'SimpleXMLElement',
													   PHPExcel_Settings::getLibXmlLoaderOptions ());
					if (is_object ($xmlTheme)) {
						...
						self::$_theme = new PHPExcel_Reader_Excel2007_Theme($themeName,
																			$colourSchemeName,
																			$themeColours);
					}
					break;
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
					$xmlCore = simplexml_load_string ($this->_getFromZipArchive ($zip,
																				 "{$rel['Target']}"),
													  'SimpleXMLElement',
													  PHPExcel_Settings::getLibXmlLoaderOptions ());
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					$xmlStrings = simplexml_load_string ($this->_getFromZipArchive ($zip,
																					"$dir/$xpath[Target]"),
														 'SimpleXMLElement',
														 PHPExcel_Settings::getLibXmlLoaderOptions ());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					...
					$xmlStyles = simplexml_load_string ($this->_getFromZipArchive ($zip,
																				   "$dir/$xpath[Target]"),
														'SimpleXMLElement',
														PHPExcel_Settings::getLibXmlLoaderOptions ()); //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					$numFmts = null;
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						...
						if ($xmlStyles->dxfs) {
							foreach ($xmlStyles->dxfs->dxf as $dxf) {
								$style = new PHPExcel_Style(FALSE,
															TRUE);
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							$fileWorksheet = $worksheets[(string)self::array_item ($eleSheet->attributes ("http://schemas.openxmlformats.org/officeDocument/2006/relationships"),
																				   "id")];
							$xmlSheet = simplexml_load_string ($this->_getFromZipArchive ($zip,
																						  "$dir/$fileWorksheet"),
															   'SimpleXMLElement',
															   PHPExcel_Settings::getLibXmlLoaderOptions ());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
								...
								foreach ($xmlSheet->sheetData->row as $row) {
									...
									foreach ($row->c as $c) {
										...
										if ($this->getReadFilter () !== NULL) {
											...
											if (!$this->getReadFilter ()->readCell ($coordinates[0],
																					$coordinates[1],
																					$docSheet->getTitle ())
											) {
												continue;
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									...
									$commentsFile = simplexml_load_string ($this->_getFromZipArchive ($zip,
																									  $relPath),
																		   'SimpleXMLElement',
																		   PHPExcel_Settings::getLibXmlLoaderOptions ());
								...
								foreach ($vmlComments as $relName => $relPath) {
									...
									foreach ($shapes as $shape) {
										...
										if (isset($shape['style'])) {
											...
											$fillColor = strtoupper (substr ((string)$shape['fillcolor'],
																			 1));
											$column = null;
											$row = null;
												...
												if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image") {
													...
													if ($this->_includeCharts) {
														$charts[self::dir_add ($fileDrawing,
																			   $ele["Target"])] = array ('id'    => (string)$ele["Id"],
							...
							if ($xmlWorkbook->definedNames) {
								foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
									...
									if (($spos = strpos ($extractedRange,
										...
										$extractedRange = substr ($extractedRange,
																  0,
																  $spos) . str_replace ('$',
																						'',
																						substr ($extractedRange,
																								$spos));
									} else {
		...
		if (!$this->_readDataOnly) {
			$contentTypes = simplexml_load_string ($this->_getFromZipArchive ($zip,
																			  "[Content_Types].xml"),
												   'SimpleXMLElement',
												   PHPExcel_Settings::getLibXmlLoaderOptions ());
					...
					case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
						if ($this->_includeCharts) {
							$chartEntryRef = ltrim ($contentType['PartName'],
													'/');
							$chartElements = simplexml_load_string ($this->_getFromZipArchive ($zip,
																							   $chartEntryRef),
																	'SimpleXMLElement',
																	PHPExcel_Settings::getLibXmlLoaderOptions ());
...
//									var_dump($chartDetails[$chartPositionRef]);
	...
	private function _readRibbon ($excel,
								  $customUITarget,
								  $zip) {
		...
		$dataRels = $this->_getFromZipArchive ($zip,
											   $pathRels);
		if ($dataRels) {
		...
		if ($localRibbon) {
			$excel->setRibbonXMLData ($customUITarget,
									  $localRibbon);
	...
	private static function _readStyle ($docStyle,
										$style) {
		// format code
...
//		}
	...
	private static function _readBorder ($docBorder,
										 $eleBorder) {
...
//		echo 'Initial Cast to Boolean', PHP_EOL;
	...
	private function _castToFormula ($c,
									 $r,
									 &$cellDataType,
									 &$value,
									 &$calculatedValue,
...
//		echo 'Initial Cast to String, PHP_EOL;
	...
	private static function dir_add ($base,
									 $add) {
		return preg_replace ('~[^/]+/\.\./~',
							 '',
							 dirname ($base) . "/$add");

...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function listWorksheetNames ($pFilename) {
		...
		while ($this->_pos < $this->_dataSize) {
			$code = self::_GetInt2d ($this->_data,
									 $this->_pos);
...
//		$this->_userDefinedProperties = $ole->getUserDefinedProperties();
	 ...
	 * @return int
	 */
	public static function _GetInt2d ($data,
									  $pos) {
	...
	private function _readBof () {
		...
		$substreamType = self::_GetInt2d ($recordData,
										  2);

		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				$version = self::_GetInt2d ($recordData,
											0);
...
//		$recordData = $this->_readRecordData($this->_data, $this->_pos + 4, $length);
	...
	private function _readSheet () {
		...
		// NOTE: not encrypted
		$rec_offset = self::_GetInt4d ($this->_data,
									   $this->_pos + 4);
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$string = $this->_readByteStringShort (substr ($recordData,
														   6));
			$rec_name = $string['value'];
		}
	 ...
	 */
	private function _readRecordData ($data,
									  $pos,
									  $len) {
		$data = substr ($data,
						$pos,
		...
		if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
			...
			if ($block != $oldBlock || $pos < $this->_rc4Pos || !$this->_rc4Key) {
				$this->_rc4Key = $this->_makeKey ($block,
												  $this->_md5Ctxt);
				$step = $pos % self::REKEY_BLOCK;
			} else {
				$step = $pos - $this->_rc4Pos;
			}
			$this->_rc4Key->RC4 (str_repeat ("\0",
											 $step));

			// Decrypt record data (re-keying at the end of every block)
			while ($block != $endBlock) {
				$step = self::REKEY_BLOCK - ($pos % self::REKEY_BLOCK);
				$recordData .= $this->_rc4Key->RC4 (substr ($data,
															0,
															$step));
				$data = substr ($data,
								$step);
	 ...
	 */
	private function _makeKey ($block,
							   $valContext) {
		$pwarray = str_repeat ("\0",
							   64);
	 ...
	 * @return int
	 */
	public static function _GetInt4d ($data,
									  $pos) {
	...
	private static function _readUnicodeStringShort ($subData) {
		...
		$string = self::_readUnicodeString (substr ($subData,
													1),
											$characterCount);
	 ...
	 * @return array
	 */
	private static function _readUnicodeString ($subData,
												$characterCount) {
		$value = '';
		...
		// needs to be fixed
		$value = self::_encodeUTF16 (substr ($subData,
											 1,
	...
	private static function _encodeUTF16 ($string,
		...
		return PHPExcel_Shared_String::ConvertEncoding ($string,
														'UTF-8',
														'UTF-16LE');
	}
	...
	private function _readByteStringShort ($subData) {
		...
		$value = $this->_decodeCodepage (substr ($subData,
												 1,
												 $ln));
			...
			while ($this->_pos <= $this->_dataSize - 4) {
				...
				switch ($code) {
					...
					case self::XLS_Type_LABEL:
						...
						$rowIndex = self::_GetInt2d ($recordData,
													 0) + 1;
						$columnIndex = self::_GetInt2d ($recordData,
														2);
	...
	public function load ($pFilename) {
		...
		if (!$this->_readDataOnly) {
			...
			foreach ($this->_phpExcel->getCellXfCollection () as $objStyle) {
				...
				if (isset($diagonal->colorIndex)) {
					$borderDiagonalColor = self::_readColor ($diagonal->colorIndex,
															 $this->_palette,
															 $this->_version);
		...
		foreach ($this->_sheets as $sheet) {
			...
			if (isset($this->_loadSheetsOnly) && !in_array ($sheet['name'],
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object ($allSpContainers[$n + 1])) {
					...
					$width = PHPExcel_Shared_Excel5::getDistanceX ($this->_phpSheet,
																   $startColumn,
																   $startOffsetX,
																   $endColumn,
																   $endOffsetX);
					$height = PHPExcel_Shared_Excel5::getDistanceY ($this->_phpSheet,
																	$startRow,
																	$startOffsetY,
																	$endRow,
																	$endOffsetY);
					...
					$offsetX = $startOffsetX * PHPExcel_Shared_Excel5::sizeCol ($this->_phpSheet,
																				$startColumn) / 1024;
					$offsetY = $startOffsetY * PHPExcel_Shared_Excel5::sizeRow ($this->_phpSheet,
																				$startRow) / 256;
...
//							echo 'Picture Object<br />';
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					...
					if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($column,
																								 $row,
																								 $this->_phpSheet->getTitle ())
					) {
						$formula = $this->_getFormulaFromStructure ($this->_sharedFormulas[$baseCell],
																	$cell);
...
//					echo '<br />';
	...
	private function _readSummaryInformation () {
		...
		$secCount = self::_GetInt4d ($this->_summaryInformation,
									 24);

		// offset: 28; size: 16; first section's class id: e0 85 9f f2 f9 4f 68 10 ab 91 08 00 2b 27 b3 d9
		// offset: 44; size: 4
		$secOffset = self::_GetInt4d ($this->_summaryInformation,
									  44);

		// section header
		// offset: $secOffset; size: 4; section length
		$secLength = self::_GetInt4d ($this->_summaryInformation,
									  $secOffset);
	...
	private function _readDocumentSummaryInformation () {
		...
		//	offset: 8;	size: 16
		//	offset: 24;	size: 4;	section count
		$secCount = self::_GetInt4d ($this->_documentSummaryInformation,
									 24);
//		echo '$secCount = ',$secCount,'<br />';

		// offset: 28;	size: 16;	first section's class id: 02 d5 cd d5 9c 2e 1b 10 93 97 08 00 2b 2c f9 ae
		// offset: 44;	size: 4;	first section offset
		$secOffset = self::_GetInt4d ($this->_documentSummaryInformation,
									  44);
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x1E:    //	null-terminated string prepended by dword string length
					$byteLength = self::_GetInt4d ($this->_documentSummaryInformation,
												   $secOffset + 4 + $offset);
	...
	private function _readCodepage () {
		...
		$codepage = self::_GetInt2d ($recordData,
									 0);
	...
	private function _readFont () {
		...
		if (!$this->_readDataOnly) {
			...
			$size = self::_GetInt2d ($recordData,
									 0);
			...
			$isItalic = (0x0002 & self::_GetInt2d ($recordData,
												   2)) >> 1;
			if ($isItalic) {
			...
			$isStrike = (0x0008 & self::_GetInt2d ($recordData,
												   2)) >> 3;
			if ($isStrike) {
			...
			$colorIndex = self::_GetInt2d ($recordData,
										   4);
			$objFont->colorIndex = $colorIndex;

			// offset: 6; size: 2; font weight
			$weight = self::_GetInt2d ($recordData,
									   6);
			switch ($weight) {
				case 0x02BC:
			...
			$escapement = self::_GetInt2d ($recordData,
										   8);
			switch ($escapement) {
				case 0x0001:
	...
	private function _readFormat () {
		...
		if (!$this->_readDataOnly) {
			$indexCode = self::_GetInt2d ($recordData,
										  0);
	...
	private static function _readUnicodeStringLong ($subData) {
		...
		$characterCount = self::_GetInt2d ($subData,
										   0);
	...
	private function _readXf () {
		...
		if (!$this->_readDataOnly) {
			...
			$numberFormatIndex = self::_GetInt2d ($recordData,
												  2);
			...
			$xfTypeProt = self::_GetInt2d ($recordData,
										   4);
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				$diagonalDown = (0x40000000 & self::_GetInt4d ($recordData,
															   10)) >> 30 ?
					true : false;

				// bit: 31; mask: 0x80000000; 1 = diagonal line from bottom left to top right
				$diagonalUp = (0x80000000 & self::_GetInt4d ($recordData,
															 10)) >> 31 ?
					true : false;
				...
				if ($fillType = self::_mapFillPattern ((0xFC000000 & self::_GetInt4d ($recordData,
																					  14)) >> 26)
				) {
				...
				$objStyle->getFill ()->startcolorIndex = (0x007F & self::_GetInt2d ($recordData,
																					18)) >> 0;

				// bit: 13-7; mask: 0x3F80; color index for pattern background
				$objStyle->getFill ()->endcolorIndex = (0x3F80 & self::_GetInt2d ($recordData,
																				  18)) >> 7;
			} else {
				// BIFF5
				...
				$borderAndBackground = self::_GetInt4d ($recordData,
														8);
				...
				$borderLines = self::_GetInt4d ($recordData,
												12);
	...
	private function _readXfExt () {
		...
		if (!$this->_readDataOnly) {
			...
			$ixfe = self::_GetInt2d ($recordData,
									 14);

			// offset: 16; size: 2; not used

			// offset: 18; size: 2; number of extension properties that follow
			$cexts = self::_GetInt2d ($recordData,
									  18);
			...
			while ($offset < $length) {
				// extension type
				$extType = self::_GetInt2d ($recordData,
											$offset);

				// extension length
				$cb = self::_GetInt2d ($recordData,
									   $offset + 2);

				// extension data
				$extData = substr ($recordData,
								   $offset + 4,
								   $cb);

				switch ($extType) {
					case 4:        // fill start color
						$xclfType = self::_GetInt2d ($extData,
													 0); // color type
						$xclrValue = substr ($extData,
											 4,
	...
	private function _readPalette () {
		...
		if (!$this->_readDataOnly) {
			...
			$nm = self::_GetInt2d ($recordData,
								   0);

			// list of RGB colors
			for ($i = 0; $i < $nm; ++$i) {
				$rgb = substr ($recordData,
							   2 + 4 * $i,
							   4);
	...
	private function _readExternalBook () {
		...
		if (strlen ($recordData) > 4) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				$externalSheetNameString = self::_readUnicodeStringLong (substr ($recordData,
																				 $offset));
	...
	private function _readExternName () {
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$options = self::_GetInt2d ($recordData,
										0);
			...
			// offset: 6; size: var
			$nameString = self::_readUnicodeStringShort (substr ($recordData,
																 6));
	...
	private function _getFormulaFromStructure ($formulaStructure,
		...
		$sz = self::_GetInt2d ($formulaStructure,
							   0);

		// offset: 2; size: sz
		$formulaData = substr ($formulaStructure,
							   2,
							   $sz);
		...
		if (strlen ($formulaStructure) > 2 + $sz) {
			$additionalData = substr ($formulaStructure,
									  2 + $sz);
		...
		return $this->_getFormulaFromData ($formulaData,
										   $additionalData,
										   $baseCell);
	}
	 ...
	 * @return string Human readable formula
	 */
	private function _getFormulaFromData ($formulaData,
										  $additionalData = '',
										  $baseCell = 'A1') {
		...
		while (strlen ($formulaData) > 0 and $token = $this->_getNextToken ($formulaData,
																			$baseCell)) {
			$tokens[] = $token;
			$formulaData = substr ($formulaData,
								   $token['size']);
	...
	private function _getNextToken ($formulaData,
		...
		switch ($id) {
			...
			case 0x1E:    //	integer
				...
				$name = 'tInt';
				$size = 3;
				$data = self::_GetInt2d ($formulaData,
										 1);
				break;
			case 0x1F:    //	number
				// offset: 1; size: 8;
				$name = 'tNum';
				$size = 9;
				$data = self::_extractNumber (substr ($formulaData,
													  1));
				$data = str_replace (',',
									 '.',
			...
			case 0x61:
				...
				switch (self::_GetInt2d ($formulaData,
										 1)) {
					case   2:
						$function = 'ISNA';
						$args = 1;
			...
			case 0x62:
				...
				$index = self::_GetInt2d ($formulaData,
										  2);
				switch ($index) {
					case   0:
						$function = 'COUNT';
			...
			case 0x63:
				...
				$definedNameIndex = self::_GetInt2d ($formulaData,
													 1) - 1;
			...
			case 0x64:
				$name = 'tRef';
				$size = 5;
				$data = $this->_readBIFF8CellAddress (substr ($formulaData,
															  1,
															  4));
				break;
			...
			case 0x66:
				...
				$subSize = self::_GetInt2d ($formulaData,
											5);
				$size = 7 + $subSize;
				$data = $this->_getFormulaFromData (substr ($formulaData,
															7,
															$subSize));
				break;
			...
			case 0x6C:
				$name = 'tRefN';
				$size = 5;
				$data = $this->_readBIFF8CellAddressB (substr ($formulaData,
															   1,
															   4),
													   $baseCell);
				break;
			...
			case 0x7A:
				...
				try {
					...
					$sheetRange = $this->_readSheetRangeByRefIndex (self::_GetInt2d ($formulaData,
																					 1));
					// offset: 3; size: 4; cell address
					$cellAddress = $this->_readBIFF8CellAddress (substr ($formulaData,
																		 3,
																		 4));
			...
			case 0x7B:
				...
				try {
					...
					$cellRangeAddress = $this->_readBIFF8CellRangeAddress (substr ($formulaData,
																				   3,
																				   8));
			...
			default:
				throw new PHPExcel_Reader_Exception('Unrecognized token ' . sprintf ('%02X',
																					 $id) . ' in formula');
				break;
		}
	...
	private static function _extractNumber ($data) {
		$rknumhigh = self::_GetInt4d ($data,
									  4);
		$rknumlow = self::_GetInt4d ($data,
									 0);
	...
	private function _readBIFF8CellAddress ($cellAddressStructure) {
		...
		$row = self::_GetInt2d ($cellAddressStructure,
								0) + 1;

		// offset: 2; size: 2; index to column or column offset + relative flags

		// bit: 7-0; mask 0x00FF; column index
		$column = PHPExcel_Cell::stringFromColumnIndex (0x00FF & self::_GetInt2d ($cellAddressStructure,
																				  2));

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d ($cellAddressStructure,
										2))
		) {
			$column = '$' . $column;
		}
		// bit: 15; mask 0x8000; (1 = relative row index, 0 = absolute row index)
		if (!(0x8000 & self::_GetInt2d ($cellAddressStructure,
										2))
		) {
			$row = '$' . $row;
		}
	...
	private function _readBIFF8CellRangeAddress ($subData) {
		...
		$fr = self::_GetInt2d ($subData,
							   0) + 1;

		// offset: 2; size: 2; index to last row (0... 65535) (or offset (-32768... 32767))
		$lr = self::_GetInt2d ($subData,
							   2) + 1;

		// offset: 4; size: 2; index to first column or column offset + relative flags

		// bit: 7-0; mask 0x00FF; column index
		$fc = PHPExcel_Cell::stringFromColumnIndex (0x00FF & self::_GetInt2d ($subData,
																			  4));

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d ($subData,
										4))
		) {
			$fc = '$' . $fc;
		}
		...
		// bit: 15; mask 0x8000; (1 = relative row index, 0 = absolute row index)
		if (!(0x8000 & self::_GetInt2d ($subData,
										4))
		) {
			$fr = '$' . $fr;
		}
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = PHPExcel_Cell::stringFromColumnIndex (0x00FF & self::_GetInt2d ($subData,
																			  6));
	...
	private function _readBIFF8CellAddressB ($cellAddressStructure,
		...
		// offset: 0; size: 2; index to row (0... 65535) (or offset (-32768... 32767))
		$rowIndex = self::_GetInt2d ($cellAddressStructure,
									 0);
		$row = self::_GetInt2d ($cellAddressStructure,
								0) + 1;
		...
		// bit: 7-0; mask 0x00FF; column index
		$colIndex = 0x00FF & self::_GetInt2d ($cellAddressStructure,
											  2);
	 ...
	 */
	private function _readBIFF8CellRangeAddressB ($subData,
												  $baseCell = 'A1') {
		list($baseCol, $baseRow) = PHPExcel_Cell::coordinateFromString ($baseCell);
		$baseCol = PHPExcel_Cell::columnIndexFromString ($baseCol) - 1;
		...
		$frIndex = self::_GetInt2d ($subData,
									0); // adjust below

		// offset: 2; size: 2; relative index to first row (0... 65535) should be treated as offset (-32768... 32767)
		$lrIndex = self::_GetInt2d ($subData,
									2); // adjust below

		// offset: 4; size: 2; first column with relative/absolute flags

		// bit: 7-0; mask 0x00FF; column index
		$fcIndex = 0x00FF & self::_GetInt2d ($subData,
											 4);
		...
		// bit: 7-0; mask 0x00FF; column index
		$lcIndex = 0x00FF & self::_GetInt2d ($subData,
											 6);
	 ...
	 * @return string Human readable formula
	 */
	private function _createFormulaFromTokens ($tokens,
											   $additionalData) {
		// empty formula?
		if (empty($tokens)) {
			return '';
			...
			switch ($token['name']) {
				...
				case 'tFuncV': // function with variable number of arguments
					if ($token['data']['function'] != '') {
						...
						$formulaStrings[] = "$space1$space0{$token['data']['function']}(" . implode (',',
																									 $ops) . ")";
						unset($space0, $space1);
					} else {
						// add-in function
						...
						$formulaStrings[] = "$space1$space0$function(" . implode (',',
																				  $ops) . ")";
						unset($space0, $space1);
					}
					break;
	...
	private static function _readBIFF8Constant ($valueData) {
		...
		switch ($identifier) {
			...
			case 0x01: // number
				...
				$value = self::_extractNumber (substr ($valueData,
													   1,
													   8));
				$size = 9;
				break;
			case 0x02: // string value
				...
				$string = self::_readUnicodeStringLong (substr ($valueData,
																1));
				$value = '"' . $string['value'] . '"';
				$size = 1 + $string['size'];
				break;
	...
	private function _readBIFF8CellRangeAddressList ($subData) {
		...
		$nm = self::_GetInt2d ($subData,
							   0);

		$offset = 2;
		// offset: 2; size: 8 * $nm; list of $nm (fixed) cell range addresses
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF8CellRangeAddressFixed (substr ($subData,
																					$offset,
																					8));
			$offset += 8;
		}
	...
	private function _readBIFF8CellRangeAddressFixed ($subData) {
		...
		$fc = self::_GetInt2d ($subData,
							   4);

		// offset: 6; size: 2; index to last column
		$lc = self::_GetInt2d ($subData,
							   6);
	...
	private function _readDefinedName () {
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$opts = self::_GetInt2d ($recordData,
									 0);
			...
			$flen = self::_GetInt2d ($recordData,
									 4);

			// offset: 8; size: 2; 0=Global name, otherwise index to sheet (1-based)
			$scope = self::_GetInt2d ($recordData,
									  8);

			// offset: 14; size: var; Name (Unicode string without length field)
			$string = self::_readUnicodeString (substr ($recordData,
														14),
												$nlen);

			// offset: var; size: $flen; formula data
			$offset = 14 + $string['size'];
			$formulaStructure = pack ('v',
									  $flen) . substr ($recordData,
													   $offset);
	...
	private function _getSplicedRecordData () {
		...
		do {
			...
			$identifier = self::_GetInt2d ($this->_data,
										   $this->_pos);
			// offset: 2; size: 2; length
			$length = self::_GetInt2d ($this->_data,
									   $this->_pos + 2);
			$data .= $this->_readRecordData ($this->_data,
											 $this->_pos + 4,
											 $length);
			...
			$this->_pos += 4 + $length;
			$nextIdentifier = self::_GetInt2d ($this->_data,
											   $this->_pos);
	...
	private function _readSst () {
		...
		for ($i = 0; $i < $nm; ++$i) {
			...
			$numChars = self::_GetInt2d ($recordData,
										 $pos);
			$pos += 2;
			...
			if ($hasRichText) {
				...
				$formattingRuns = self::_GetInt2d ($recordData,
												   $pos);
				$pos += 2;
			}
			...
			if ($hasAsian) {
				...
				$extendedRunLength = self::_GetInt4d ($recordData,
													  $pos);
				$pos += 4;
			}
			...
			// convert to UTF-8
			$retstr = self::_encodeUTF16 ($retstr,
										  $isCompressed);
			...
			if ($hasRichText) {
				...
				for ($j = 0; $j < $formattingRuns; ++$j) {
					...
					$charPos = self::_GetInt2d ($recordData,
												$pos + $j * 4);
	...
	private function _readPrintGridlines () {
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			$printGridlines = (bool)self::_GetInt2d ($recordData,
													 0);
	...
	private function _readDefaultRowHeight () {
		...
		$height = self::_GetInt2d ($recordData,
								   2);
	...
	private function _readSheetPr () {
		...
		$isSummaryBelow = (0x0040 & self::_GetInt2d ($recordData,
													 0)) >> 6;
		$this->_phpSheet->setShowSummaryBelow ($isSummaryBelow);

		// bit: 7; mask: 0x0080; 0 = outline buttons left of outline group
		$isSummaryRight = (0x0080 & self::_GetInt2d ($recordData,
													 0)) >> 7;
		$this->_phpSheet->setShowSummaryRight ($isSummaryRight);

		// bit: 8; mask: 0x100; 0 = scale printout in percent, 1 = fit printout to number of pages
		// this corresponds to radio button setting in page setup dialog in Excel
		$this->_isFitToPages = (bool)((0x0100 & self::_GetInt2d ($recordData,
																 0)) >> 8);
	}
	...
	private function _readHorizontalPageBreaks () {
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				$r = self::_GetInt2d ($recordData,
									  2 + 6 * $i);
				$cf = self::_GetInt2d ($recordData,
									   2 + 6 * $i + 2);
				$cl = self::_GetInt2d ($recordData,
									   2 + 6 * $i + 4);

				// not sure why two column indexes are necessary?
				$this->_phpSheet->setBreakByColumnAndRow ($cf,
														  $r,
	...
	private function _readVerticalPageBreaks () {
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			// offset: 2; size: 6 * $nm; list of $nm row index structures
			for ($i = 0; $i < $nm; ++$i) {
				$c = self::_GetInt2d ($recordData,
									  2 + 6 * $i);
				$rf = self::_GetInt2d ($recordData,
									   2 + 6 * $i + 2);
				$rl = self::_GetInt2d ($recordData,
									   2 + 6 * $i + 4);

				// not sure why two row indexes are necessary?
				$this->_phpSheet->setBreakByColumnAndRow ($c,
														  $rf,
	...
	private function _readHcenter () {
		...
		if (!$this->_readDataOnly) {
			...
			$isHorizontalCentered = (bool)self::_GetInt2d ($recordData,
														   0);
	...
	private function _readVcenter () {
		...
		if (!$this->_readDataOnly) {
			...
			$isVerticalCentered = (bool)self::_GetInt2d ($recordData,
														 0);
	...
	private function _readPageSetup () {
		...
		if (!$this->_readDataOnly) {
			...
			$paperSize = self::_GetInt2d ($recordData,
										  0);

			// offset: 2; size: 2; scaling factor
			$scale = self::_GetInt2d ($recordData,
									  2);

			// offset: 6; size: 2; fit worksheet width to this number of pages, 0 = use as many as needed
			$fitToWidth = self::_GetInt2d ($recordData,
										   6);

			// offset: 8; size: 2; fit worksheet height to this number of pages, 0 = use as many as needed
			$fitToHeight = self::_GetInt2d ($recordData,
											8);

			// offset: 10; size: 2; option flags

			// bit: 1; mask: 0x0002; 0=landscape, 1=portrait
			$isPortrait = (0x0002 & self::_GetInt2d ($recordData,
													 10)) >> 1;

			// bit: 2; mask: 0x0004; 1= paper size, scaling factor, paper orient. not init
			// when this bit is set, do not use flags for those properties
			$isNotInit = (0x0004 & self::_GetInt2d ($recordData,
													10)) >> 2;
			...
			$marginHeader = self::_extractNumber (substr ($recordData,
														  16,
														  8));
			$this->_phpSheet->getPageMargins ()->setHeader ($marginHeader);

			// offset: 24; size: 8; footer margin (IEEE 754 floating-point value)
			$marginFooter = self::_extractNumber (substr ($recordData,
														  24,
														  8));
	...
	private function _readProtect () {
		...
		$bool = (0x01 & self::_GetInt2d ($recordData,
										 0)) >> 0;
	...
	private function _readPassword () {
		...
		if (!$this->_readDataOnly) {
			...
			$this->_phpSheet->getProtection ()->setPassword ($password,
															 true);
		}
	}
	...
	private function _readDefColWidth () {
		...
		$width = self::_GetInt2d ($recordData,
								  0);
		if ($width != 8) {
	...
	private function _readColInfo () {
		...
		if (!$this->_readDataOnly) {
			...
			$fc = self::_GetInt2d ($recordData,
								   0); // first column index

			// offset: 2; size: 2; index to last column in range
			$lc = self::_GetInt2d ($recordData,
			...
			$xfIndex = self::_GetInt2d ($recordData,
										6);

			// offset: 8; size: 2; option flags

			// bit: 0; mask: 0x0001; 1= columns are hidden
			$isHidden = (0x0001 & self::_GetInt2d ($recordData,
												   8)) >> 0;

			// bit: 10-8; mask: 0x0700; outline level of the columns (0 = no outline)
			$level = (0x0700 & self::_GetInt2d ($recordData,
												8)) >> 8;

			// bit: 12; mask: 0x1000; 1 = collapsed
			$isCollapsed = (0x1000 & self::_GetInt2d ($recordData,
													  8)) >> 12;
	...
	private function _readRow () {
		...
		if (!$this->_readDataOnly) {
			...
			$height = (0x7FFF & self::_GetInt2d ($recordData,
												 6)) >> 0;

			// bit: 15: mask: 0x8000; 0 = row has custom height; 1= row has default height
			$useDefaultHeight = (0x8000 & self::_GetInt2d ($recordData,
														   6)) >> 15;
			...
			$level = (0x00000007 & self::_GetInt4d ($recordData,
													12)) >> 0;
			$this->_phpSheet->getRowDimension ($r + 1)->setOutlineLevel ($level);

			// bit: 4; mask: 0x00000010; 1 = outline group start or ends here... and is collapsed
			$isCollapsed = (0x00000010 & self::_GetInt4d ($recordData,
														  12)) >> 4;
			$this->_phpSheet->getRowDimension ($r + 1)->setCollapsed ($isCollapsed);

			// bit: 5; mask: 0x00000020; 1 = row is hidden
			$isHidden = (0x00000020 & self::_GetInt4d ($recordData,
													   12)) >> 5;
			$this->_phpSheet->getRowDimension ($r + 1)->setVisible (!$isHidden);

			// bit: 7; mask: 0x00000080; 1 = row has explicit format
			$hasExplicitFormat = (0x00000080 & self::_GetInt4d ($recordData,
																12)) >> 7;

			// bit: 27-16; mask: 0x0FFF0000; only applies when hasExplicitFormat = 1; index to XF record
			$xfIndex = (0x0FFF0000 & self::_GetInt4d ($recordData,
													  12)) >> 16;
	...
	private function _readRk () {
		...
		$row = self::_GetInt2d ($recordData,
								0);

		// offset: 2; size: 2; index to column
		$column = self::_GetInt2d ($recordData,
								   2);
		...
		if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($columnString,
			...
			$rknum = self::_GetInt4d ($recordData,
									  6);
	...
	private function _readLabelSst () {
		...
		if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($columnString,
			...
			$index = self::_GetInt4d ($recordData,
									  6);
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = PHPExcel_Shared_String::Substring ($this->_sst[$index]['value'],
																   $charPos,
	...
	private function _readMulRk () {
		...
		$colFirst = self::_GetInt2d ($recordData,
									 2);

		// offset: var; size: 2; index to last column
		$colLast = self::_GetInt2d ($recordData,
									$length - 2);
	...
	private function _readNumber () {
		...
		if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($columnString,
			...
			$numValue = self::_extractNumber (substr ($recordData,
													  6,
													  8));
	...
	private function _readFormula () {
		...
		$formulaStructure = substr ($recordData,
									20);
		...
		if ($isPartOfSharedFormula) {
			...
			$baseRow = self::_GetInt2d ($formulaStructure,
										3);
			$baseCol = self::_GetInt2d ($formulaStructure,
										5);
		...
		if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($columnString,
			...
			if ((ord ($recordData{6}) == 0)
				...
				$value = self::_extractNumber (substr ($recordData,
													   6,
													   8));
	...
	private function _readLabel () {
		...
		if (($this->getReadFilter () !== NULL) && $this->getReadFilter ()->readCell ($columnString,
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				$string = $this->_readByteStringLong (substr ($recordData,
															  6));
				$value = $string['value'];
			}
			$cell = $this->_phpSheet->getCell ($columnString . ($row + 1));
	...
	private function _readObj () {
		...
		$ftCmoType = self::_GetInt2d ($recordData,
									  0);
		$cbCmoSize = self::_GetInt2d ($recordData,
									  2);
		$otObjType = self::_GetInt2d ($recordData,
									  4);
		$idObjID = self::_GetInt2d ($recordData,
									6);
		$grbitOpts = self::_GetInt2d ($recordData,
									  6);
...
//		echo '<br />';
	...
	private function _readWindow2 () {
		...
		$firstVisibleRow = self::_GetInt2d ($recordData,
											2);

		// offset: 4; size: 2; index to first visible colum
		$firstVisibleColumn = self::_GetInt2d ($recordData,
											   4);
		if ($this->_version === self::XLS_BIFF8) {
			// offset:  8; size: 2; not used
			...
			$zoomscaleInPageBreakPreview = self::_GetInt2d ($recordData,
															10);
			if ($zoomscaleInPageBreakPreview === 0) {
				$zoomscaleInPageBreakPreview = 60;
			}
			$zoomscaleInNormalView = self::_GetInt2d ($recordData,
													  12);
	...
	private function _readPageLayoutView () {
		...
		// offset: 0; size: 2; rt
		//->ignore
		$rt = self::_GetInt2d ($recordData,
							   0);
		// offset: 2; size: 2; grbitfr
		//->ignore
		$grbitFrt = self::_GetInt2d ($recordData,
									 2);
		// offset: 4; size: 8; reserved
		//->ignore

		// offset: 12; size 2; zoom scale
		$wScalePLV = self::_GetInt2d ($recordData,
									  12);
		// offset: 14; size 2; grbit
		$grbit = self::_GetInt2d ($recordData,
								  14);
	...
	private function _readScl () {
		...
		$numerator = self::_GetInt2d ($recordData,
									  0);

		// offset: 2; size: 2; numerator of the view magnification
		$denumerator = self::_GetInt2d ($recordData,
										2);
	...
	private function _readPane () {
		...
		if (!$this->_readDataOnly) {
			...
			$px = self::_GetInt2d ($recordData,
								   0);

			// offset: 2; size: 2; position of horizontal split
			$py = self::_GetInt2d ($recordData,
								   2);
	...
	private function _readSelection () {
		...
		if (!$this->_readDataOnly) {
			...
			$index = self::_GetInt2d ($recordData,
									  5);

			// offset: 7; size: var; cell range address list containing all selected cell ranges
			$data = substr ($recordData,
							7);
	...
	private function _readBIFF5CellRangeAddressList ($subData) {
		...
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF5CellRangeAddressFixed (substr ($subData,
																					$offset,
																					6));
			$offset += 6;
		}
	...
	private function _readHyperLink () {
		...
		if (!$this->_readDataOnly) {
			...
			$isFileLinkOrUrl = (0x00000001 & self::_GetInt2d ($recordData,
															  28)) >> 0;

			// bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or URL
			$isAbsPathOrUrl = (0x00000001 & self::_GetInt2d ($recordData,
															 28)) >> 1;

			// bit: 2 (and 4); mask: 0x00000014; 0 = no description
			$hasDesc = (0x00000014 & self::_GetInt2d ($recordData,
													  28)) >> 2;

			// bit: 3; mask: 0x00000008; 0 = no text, 1 = has text
			$hasText = (0x00000008 & self::_GetInt2d ($recordData,
													  28)) >> 3;

			// bit: 7; mask: 0x00000080; 0 = no target frame, 1 = has target frame
			$hasFrame = (0x00000080 & self::_GetInt2d ($recordData,
													   28)) >> 7;

			// bit: 8; mask: 0x00000100; 0 = file link or URL, 1 = UNC path (inc. server name)
			$isUNC = (0x00000100 & self::_GetInt2d ($recordData,
													28)) >> 8;
			...
			if ($hasDesc) {
				...
				$dl = self::_GetInt4d ($recordData,
									   32);
				// offset: 36; size: var; character array of description text, no Unicode string header, always 16-bit characters, zero terminated
				$desc = self::_encodeUTF16 (substr ($recordData,
													36,
													2 * ($dl - 1)),
											false);
				$offset += 4 + 2 * $dl;
			...
			switch ($hyperlinkType) {
				...
				case 'local':
					...
					$upLevelCount = self::_GetInt2d ($recordData,
													 $offset);
					$offset += 2;

					// offset: var; size: 4; character count of the shortened file path and name, including trailing zero word
					$sl = self::_GetInt4d ($recordData,
										   $offset);
					$offset += 4;

					// offset: var; size: sl; character array of the shortened file path and name in 8.3-DOS-format (compressed Unicode string)
					$shortenedFilePath = substr ($recordData,
												 $offset,
												 $sl);
					$shortenedFilePath = self::_encodeUTF16 ($shortenedFilePath,
															 true);
					$shortenedFilePath = substr ($shortenedFilePath,
												 0,
					...
					if ($sz > 0) {
						...
						$extendedFilePath = substr ($recordData,
													$offset,
													$xl);
						$extendedFilePath = self::_encodeUTF16 ($extendedFilePath,
																false);
						$offset += $xl;
					}
			...
			if ($hasText) {
				...
				$tl = self::_GetInt4d ($recordData,
									   $offset);
				$offset += 4;
				// offset: var; size: var; character array of the text mark without the # sign, no Unicode header, always 16-bit characters, zero-terminated
				$text = self::_encodeUTF16 (substr ($recordData,
													$offset,
													2 * ($tl - 1)),
											false);
				$url .= $text;
	...
	private function _readDataValidation () {
		...
		$options = self::_GetInt4d ($recordData,
									0);
		...
		$formula1 = substr ($recordData,
							$offset,
							$sz1);
		$formula1 = pack ('v',
						  $sz1) . $formula1; // prepend the length
		try {
		...
		$formula2 = substr ($recordData,
							$offset,
							$sz2);
		$formula2 = pack ('v',
						  $sz2) . $formula2; // prepend the length
		try {
		...
		$cellRangeAddressList = $this->_readBIFF8CellRangeAddressList (substr ($recordData,
																			   $offset));
		...
		if (!$this->_readDataOnly) {
			...
			switch ($sz) {
				case 0x14:
					...
					$colorIndex = self::_GetInt2d ($recordData,
												   16);
					$color = self::_readColor ($colorIndex,
											   $this->_palette,
											   $this->_version);
	...
	private function _readRangeProtection () {
		...
		if (!$this->_readDataOnly) {
			...
			$cref = self::_GetInt2d ($recordData,
									 19);
			$offset += 2;
			...
			for ($i = 0; $i < $cref; ++$i) {
				try {
					$cellRange = $this->_readBIFF8CellRangeAddressFixed (substr ($recordData,
																				 27 + 8 * $i,
																				 8));
				}
				catch (PHPExcel_Exception $e) {
			...
			$rgbFeat = substr ($recordData,
							   $offset);
			$offset += 4;

			// offset: var; size: 4; the encrypted password (only 16-bit although field is 32-bit)
			$wPassword = self::_GetInt4d ($recordData,
										  $offset);
			$offset += 4;

			// Apply range protection to sheet
			if ($cellRanges) {
				$this->_phpSheet->protectCells (implode (' ',
														 $cellRanges),
												strtoupper (dechex ($wPassword)),
												true);
			}
		}
	}
...
//		echo '<b>Read Cell Annotation</b><br />';
		...
		$cellAddress = $this->_readBIFF8CellAddress (substr ($recordData,
															 0,
															 4));
		if ($this->_version == self::XLS_BIFF8) {
			$noteObjID = self::_GetInt2d ($recordData,
										  6);
			$noteAuthor = self::_readUnicodeStringLong (substr ($recordData,
																8));
...
//			echo 'Note Address=',$cellAddress,'<br />';
			...
			$cellAddress = str_replace ('$',
										'',
										$cellAddress);
			$noteLength = self::_GetInt2d ($recordData,
										   4);
			$noteText = trim (substr ($recordData,
									  6));
...
//													->setAuthor( $author )
	...
	private function _readTextObject () {
		...
		$grbitOpts = self::_GetInt2d ($recordData,
									  0);
		$rot = self::_GetInt2d ($recordData,
								2);
		$cchText = self::_GetInt2d ($recordData,
									10);
		$cbRuns = self::_GetInt2d ($recordData,
								   12);
...
//		echo '<br />';
	...
	private function _readContinue () {
		...
		$splitPoint = self::_GetInt2d ($recordData,
									   2);
		if (in_array ($splitPoint,
					  $validSplitPoints)) {
	...
	private function _readImData () {
		...
		$env = self::_GetInt2d ($recordData,
								2);

		// offset: 4; size: 4; length of the image data
		$lcb = self::_GetInt4d ($recordData,
								4);

		// offset: 8; size: var; image data
		$iData = substr ($recordData,
						 8);
...
//			var_dump($bcHeight);
				...
				$bcBitCount = self::_GetInt2d ($iData,
											   10);
//			var_dump($bcBitCount);

				$rgbString = substr ($iData,
									 12);
				$rgbTriples = array ();
				while (strlen ($rgbString) > 0) {
					$rgbTriples[] = unpack ('Cb/Cg/Cr',
											$rgbString);
					$rgbString = substr ($rgbString,
										 3);
				}
				$x = 0;
				$y = 0;
				foreach ($rgbTriples as $i => $rgbTriple) {
					$color = imagecolorallocate ($ih,
												 $rgbTriple['r'],
												 $rgbTriple['g'],
												 $rgbTriple['b']);
					imagesetpixel ($ih,

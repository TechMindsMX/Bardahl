...
class PHPExcel_Reader_HTML extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function loadIntoExisting ($pFilename,
		...
		if ($loaded === FALSE) {
			throw new PHPExcel_Reader_Exception('Failed to load ',
												$pFilename,
												' as a DOM Document');
		}
...
//		var_dump($this->_dataArray);
	...
	private function _processDomElement (DOMNode $element,
										 $sheet,
										 &$row,
										 &$column,
										 &$cellContent) {
...
//						echo 'END OF TABLE ' , $this->_tableLevel , ' CELL<br />';
	...
	private function _flushCell ($sheet,
								 $column,
								 $row,
								 &$cellContent) {
		if (is_string ($cellContent)) {
...
//				echo 'FLUSH CELL: ' , $column , $row , ' => ' , $cellContent , '<br />';
				...
				$cell = $sheet->setCellValue ($column . $row,
											  $cellContent,
											  true);

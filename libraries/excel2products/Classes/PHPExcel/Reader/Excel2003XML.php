...
class PHPExcel_Reader_Excel2003XML extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function listWorksheetNames ($pFilename) {
		...
		foreach ($xml_ss->Worksheet as $worksheet) {
			...
			$worksheetNames[] = self::_convertStringEncoding ((string)$worksheet_ss['Name'],
															  $this->_charSet);
		}
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	private static function _convertStringEncoding ($string,
													$charset) {
		if ($charset != 'UTF-8') {
...
//												$thisBorder['style'] = $borderStyleValue;
									...
									case 'Position' :
										...
										break;
									case 'Color' :
										$borderColour = substr ($borderStyleValue,
																1);
...
//
			...
			if (isset($worksheet_ss['Name'])) {
				$worksheetName = self::_convertStringEncoding ((string)$worksheet_ss['Name'],
															   $this->_charSet);
						...
						if (isset($cell->Data)) {
							...
							if (isset($cellData_ss['Type'])) {
								...
								switch ($cellDataType) {
									...
									*/
									case 'String' :
										$cellValue = self::_convertStringEncoding ($cellValue,
																				   $this->_charSet);
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
	...
	private function _parseRichText ($is = '') {
		...
		$value->createText (self::_convertStringEncoding ($is,
														  $this->_charSet));

...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
	...
	public static function toFormattedString ($value = '0',
		...
		$color_regex = '/^\\[[a-zA-Z]+\\]/';
		$format = preg_replace ($color_regex,
								'',
								$format);
		...
		if (preg_match ('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i',
			...
			self::_formatAsDate ($value,
								 $format);
		} else {
			if (preg_match ('/%$/',
							$format)) { // % number format
				self::_formatAsPercentage ($value,
										   $format);
			} else {
				if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
					$value = 'EUR ' . sprintf ('%1.2f',
											   $value);
				} else {
					// In Excel formats, "_" is used to add spacing, which we can't do in HTML
					$format = preg_replace ('/_./',
											'',
											$format);
					...
					if (preg_match ('/#?.*\?\/\?/',
						...
						if (preg_match ($number_regex,
							...
							if ($useThousands) {
								...
								if (preg_match ('/[0#]E[+-]0/i',
												$format)) {
									//	Scientific format
									$value = sprintf ('%5.2E',
													  $value);
								} elseif (preg_match ('/0([^\d\.]+)0/',
													  $format)) {
		...
		if ($callBack !== null) {
			...
			$value = $writerInstance->$function($value,
												$formatColor);
		}
	...
	private static function _formatAsDate (&$value,
										   &$format) {
		...
		$format = preg_replace ('/^(\[\$[A-Z]*-[0-9A-F]*\])/i',
								'',
								$format);
		...
		$format = strtr ($format,
						 self::$_dateFormatReplacements);
		if (!strpos ($format,
					 'A')
		) {    // 24-hour time format
			$format = strtr ($format,
							 self::$_dateFormatReplacements24);
		} else {                    // 12-hour time format
			$format = strtr ($format,
							 self::$_dateFormatReplacements12);
		}
	...
	private static function _formatAsPercentage (&$value,
												 &$format) {
	...
	private static function _formatAsFraction (&$value,
											   &$format) {
		$sign = ($value < 0) ? '-' : '';

		$integerPart = floor (abs ($value));
		$decimalPart = trim (fmod (abs ($value),
								   1),
							 '0.');
		$decimalLength = strlen ($decimalPart);
		$decimalDivisor = pow (10,
							   $decimalLength);
	...
	private static function _complexNumberFormatMask ($number,
													  $mask) {
		if (strpos ($mask,
					'.') !== false
		) {
			...
			$masks = explode ('.',
							  $mask . '.0');
			$result1 = self::_complexNumberFormatMask ($numbers[0],
													   $masks[0]);
			$result2 = strrev (self::_complexNumberFormatMask (strrev ($numbers[1]),
															   strrev ($masks[1])));

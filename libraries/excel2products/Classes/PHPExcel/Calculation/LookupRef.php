...
class PHPExcel_Calculation_LookupRef
{
	 ...
	 * @return    string
	 */
	public static function CELL_ADDRESS ($row,
										 $column,
										 $relativity = 1,
										 $referenceStyle = True,
										 $sheetText = '') {
	...
	public static function COLUMN ($cellAddress = Null) {
		...
		if (is_array ($cellAddress)) {
			...
			if (strpos ($cellAddress,
				...
				list($sheet, $cellAddress) = explode ('!',
													  $cellAddress);
			}
			if (strpos ($cellAddress,
						':') !== false
			) {
				list($startAddress, $endAddress) = explode (':',
															$cellAddress);
		...
		if (is_array ($cellAddress)) {
			foreach ($cellAddress as $columnKey => $rowValue) {
				foreach ($rowValue as $rowKey => $cellValue) {
					return (integer)preg_replace ('/[^0-9]/i',
												  '',
												  $rowKey);
				}
			}
			...
			if (strpos ($cellAddress,
				...
				list($startAddress, $endAddress) = explode (':',
															$cellAddress);
				$startAddress = preg_replace ('/[^0-9]/',
											  '',
											  $startAddress);
				$endAddress = preg_replace ('/[^0-9]/',
											'',
											$endAddress);
				$returnValue = array ();
				do {
				...
				list($cellAddress) = explode (':',
											  $cellAddress);
				return (integer)preg_replace ('/[^0-9]/',
											  '',
											  $cellAddress);
			}
		}
	 ...
	 */
	public static function HYPERLINK ($linkURL = '',
									  $displayName = null,
	 ...
	 *
	 */
	public static function INDIRECT ($cellAddress = NULL,
									 PHPExcel_Cell $pCell = NULL) {
		...
		if (strpos ($cellAddress,
			...
			list($cellAddress1, $cellAddress2) = explode (':',
														  $cellAddress);
		}
		...
		if ((!preg_match ('/^' . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF . '$/i',
			...
			if (!preg_match ('/^' . PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE . '$/i',
							 $cellAddress1,
							 $matches)
			) {
				return PHPExcel_Calculation_Functions::REF ();
			...
			if (strpos ($cellAddress,
				...
				list($sheetName, $cellAddress) = explode ('!',
														  $cellAddress);
			...
			return PHPExcel_Calculation::getInstance ()->extractNamedRange ($cellAddress,
																			$pSheet,
																			FALSE);
		}
		...
		return PHPExcel_Calculation::getInstance ()->extractCellRange ($cellAddress,
																	   $pSheet,
																	   FALSE);
	}    //	function INDIRECT()
	 ...
	 */
	public static function OFFSET ($cellAddress = Null,
								   $rows = 0,
								   $columns = 0,
								   $height = null,
								   $width = null) {
		...
		if (strpos ($cellAddress,
					":")) {
			list($startCell, $endCell) = explode (":",
												  $cellAddress);
		} else {
	 ...
	 */
	public static function MATCH ($lookup_value,
								  $lookup_array,
								  $match_type = 1) {
		...
		foreach ($lookup_array as $i => $lookupArrayValue) {
			...
			if ((is_null ($lookupArrayValue)) && (($match_type == 1) || ($match_type == -1))) {
				$lookup_array = array_slice ($lookup_array,
											 0,
											 $i - 1);
			}
		}
...
//				echo '$i='.$i.'<br />';
	 ...
	 */
	public static function INDEX ($arrayValues,
								  $rowNum = 0,
								  $columnNum = 0) {
	 ...
	 */
	public static function HLOOKUP ($lookup_value,
									$lookup_array,
									$index_number,
	 ...
	 * @return    mixed            The value of the found cell
	 */
	public static function LOOKUP ($lookup_value,
								   $lookup_vector,
								   $result_vector = null) {
		$lookup_value = PHPExcel_Calculation_Functions::flattenSingleValue ($lookup_value);
		...
		return self::VLOOKUP ($lookup_value,
							  $lookup_vector,
							  2);
	 ...
	 * @return    mixed            The value of the found cell
	 */
	public static function VLOOKUP ($lookup_value,
									$lookup_array,
									$index_number,
									$not_exact_match = true) {
		$lookup_value = PHPExcel_Calculation_Functions::flattenSingleValue ($lookup_value);
	...
	private static function _vlookupSort ($a,
										  $b) {
		$f = array_keys ($a);
		$firstColumn = array_shift ($f);

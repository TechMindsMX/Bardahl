...
class PHPExcel_Calculation_Engineering
{
	 ...
	 *
	 */
	public static function BESSELK ($x,
									$ord) {
	...
	private static function _Besselk0 ($fNum) {
		if ($fNum <= 2) {
			$fNum2 = $fNum * 0.5;
			$y = ($fNum2 * $fNum2);
			$fRet = -log ($fNum2) * self::BESSELI ($fNum,
												   0) +
	 ...
	 *
	 */
	public static function BESSELI ($x,
									$ord) {
		$x = (is_null ($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($x);
		$ord = (is_null ($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($ord);
		...
		if ((is_numeric ($x)) && (is_numeric ($ord))) {
			...
			if (abs ($x) <= 30) {
				$fResult = $fTerm = pow ($x / 2,
										 $ord) / PHPExcel_Calculation_MathTrig::FACT ($ord);
				$ordK = 1;
				$fSqrX = ($x * $x) / 4;
				do {
					$fTerm *= $fSqrX;
	 ...
	 * @return    float
	 */
	public static function BESSELY ($x,
									$ord) {
		$x = (is_null ($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($x);
		$ord = (is_null ($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($ord);
	...
	private static function _Bessely0 ($fNum) {
		if ($fNum < 8.0) {
			...
			$fRet = $f1 / $f2 + 0.636619772 * self::BESSELJ ($fNum,
															 0) * log ($fNum);
		} else {
			$z = 8.0 / $fNum;
			$y = ($z * $z);
	 ...
	 *
	 */
	public static function BESSELJ ($x,
									$ord) {
		$x = (is_null ($x)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($x);
		$ord = (is_null ($ord)) ? 0.0 : PHPExcel_Calculation_Functions::flattenSingleValue ($ord);
	...
	public static function BINTOHEX ($x,
		...
		if (strlen ($x) > 10) {
			...
			//	Two's Complement
			return str_repeat ('F',
							   8) . substr (strtoupper (dechex (bindec (substr ($x,
																				-9)))),
											-2);
		}
		$hexVal = (string)strtoupper (dechex (bindec ($x)));

		return self::_nbrConversionFormat ($hexVal,
										   $places);
	}    //	function BINTODEC()
	 ...
	 */
	private static function _nbrConversionFormat ($xVal,
												  $places) {
		if (!is_null ($places)) {
			if (strlen ($xVal) <= $places) {
				return substr (str_pad ($xVal,
										$places,
										'0',
										STR_PAD_LEFT),
							   -10);
	...
	public static function BINTOOCT ($x,
		...
		if (strlen ($x) > 10) {
			...
			//	Two's Complement
			return str_repeat ('7',
							   7) . substr (strtoupper (decoct (bindec (substr ($x,
																				-9)))),
											-3);
		}
		$octVal = (string)decoct (bindec ($x));

		return self::_nbrConversionFormat ($octVal,
										   $places);
	}    //	function BINTOOCT()
	...
	public static function DECTOBIN ($x,
		...
		return self::_nbrConversionFormat ($r,
										   $places);
	}    //	function DECTOBIN()
	...
	public static function HEXTOBIN ($x,
		...
		$x = (string)$x;
		if (strlen ($x) > preg_match_all ('/[0123456789ABCDEF]/',
										  strtoupper ($x),
										  $out)
		) {
			return PHPExcel_Calculation_Functions::NaN ();
		...
		return substr (self::_nbrConversionFormat ($binVal,
												   $places),
					   -10);
	}    //	function HEXTOBIN()
	...
	public static function _parseComplex ($complexNumber) {
		...
		$suffix = substr ($workString,
						  -1);
		if (!is_numeric ($suffix)) {
			$workString = substr ($workString,
								  0,
								  -1);
		} else {
			$suffix = '';
		...
		$realNumber = strtok ($workString,
							  '+-');
		if (strtoupper (substr ($realNumber,
								-1)) == 'E'
		) {
			$power = strtok ('+-');
			++$leadingSign;
		...
		$realNumber = substr ($workString,
							  0,
							  strlen ($realNumber) + strlen ($power) + $leadingSign);

		if ($suffix != '') {
			$imaginary = substr ($workString,
								 strlen ($realNumber));
	...
	private static function _cleanComplex ($complexNumber) {
		if ($complexNumber{0} == '+') {
			$complexNumber = substr ($complexNumber,
									 1);
		}
	 ...
	 * @return    string
	 */
	public static function COMPLEX ($realNumber = 0.0,
									$imaginary = 0.0,
									$suffix = 'i') {
	...
	public static function IMLOG10 ($complexNumber) {
		...
		return self::IMPRODUCT (log10 (EULER),
								self::IMLN ($complexNumber));
	}    //	function IMLOG2()
	...
	public static function IMLOG2 ($complexNumber) {
		...
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) {
			...
			return log ($parsedComplex['real'],
						2);
		}

		return self::IMPRODUCT (log (EULER,
									 2),
								self::IMLN ($complexNumber));
	}    //	function IMDIV()
	 ...
	 * @return    string
	 */
	public static function IMPOWER ($complexNumber,
									$realNumber) {
		$complexNumber = PHPExcel_Calculation_Functions::flattenSingleValue ($complexNumber);
		...
		if ($theta == 0) {
			...
			return self::COMPLEX ($rPower * cos ($theta),
								  $rPower * sin ($theta),
								  $parsedComplex['suffix']);
		} else {
			return self::COMPLEX ($rPower * cos ($theta),
	 ...
	 * @return    string
	 */
	public static function IMDIV ($complexDividend,
								  $complexDivisor) {
	 ...
	 * @return    string
	 */
	public static function IMSUB ($complexNumber1,
								  $complexNumber2) {
		...
		return self::COMPLEX ($d1,
							  $d2,
							  $parsedComplex1['suffix']);
	}    //	function DELTA()
	 ...
	 * @return    int
	 */
	public static function GESTEP ($number,
								   $step = 0) {
	 ...
	 * @return    float
	 */
	public static function ERF ($lower,
								$upper = NULL) {
	 ...
	 * @return    float
	 */
	public static function CONVERTUOM ($value,
									   $fromUOM,
									   $toUOM) {
		...
		if (isset(self::$_conversionUnits[$fromUOM])) {
			...
			$fromMultiplier = substr ($fromUOM,
									  0,
									  1);
			$fromUOM = substr ($fromUOM,
							   1);

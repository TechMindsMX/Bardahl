...
class PHPExcel_Shared_JAMA_Matrix
{
	 ...
	 */
	public function getMatrixByRow ($i0 = null,
									$iF = null) {
		if (is_int ($i0)) {
			if (is_int ($iF)) {
				return $this->getMatrix ($i0,
										 0,
										 $iF + 1,
										 $this->n);
			} else {
		...
		if (func_num_args () > 0) {
			...
			switch ($match) {
				...
				case 'integer,integer':
					...
					$R = new PHPExcel_Shared_JAMA_Matrix($m,
														 $n);
					for ($i = $i0; $i < $this->m; ++$i) {
						for ($j = $j0; $j < $this->n; ++$j) {
							$R->set ($i,
				...
				case 'integer,integer,integer,integer':
					...
					$R = new PHPExcel_Shared_JAMA_Matrix($m + 1,
														 $n + 1);
					for ($i = $i0; $i <= $iF; ++$i) {
						for ($j = $j0; $j <= $jF; ++$j) {
							$R->set ($i - $i0,
	 ...
	 * @return Matrix Submatrix
	 */
	public function getMatrixByCol ($j0 = null,
									$jF = null) {
		if (is_int ($j0)) {
			if (is_int ($jF)) {
	...
	public function transpose () {
		$R = new PHPExcel_Shared_JAMA_Matrix($this->n,
											 $this->m);
		for ($i = 0; $i < $this->m; ++$i) {
			for ($j = 0; $j < $this->n; ++$j) {
				$R->set ($j,
		...
		if (func_num_args () > 0) {
			...
			switch ($match) {
				case 'object':
					...
					if ($this->n == $B->m) {
						$C = new PHPExcel_Shared_JAMA_Matrix($this->m,
															 $B->n);
						for ($j = 0; $j < $B->n; ++$j) {
							for ($k = 0; $k < $this->n; ++$k) {
								$Bcolj[$k] = $B->A[$k][$j];
	 ...
	 */
	public function identity ($m = null,
							  $n = null) {
		return $this->diagonal ($m,
								$n,
								1);
	}    //	function solve()
	 ...
	 */
	public function diagonal ($m = null,
							  $n = null,
							  $c = 1) {
		$R = new PHPExcel_Shared_JAMA_Matrix($m,
											 $n);

...
class PHPExcel_Worksheet_AutoFilter
{
	 ...
	 * @return boolean
	 */
	private static function _filterTestInSimpleDataSet ($cellValue,
														$dataSet) {
	 ...
	 * @return boolean
	 */
	private static function _filterTestInDateGroupSet ($cellValue,
													   $dataSet) {
	 ...
	 * @return boolean
	 */
	private static function _filterTestInCustomDataSet ($cellValue,
														$ruleSet) {
		...
		foreach ($dataSet as $rule) {
			if (is_numeric ($rule['value'])) {
				...
				$retVal = preg_match ('/^' . $rule['value'] . '$/i',
									  $cellValue);
			}
	 ...
	 * @return boolean
	 */
	private static function _filterTestInPeriodDateSet ($cellValue,
														$monthSet) {
		//	Blank cells are always ignored, so return a FALSE
		if (($cellValue == '') || ($cellValue === NULL)) {
			return FALSE;
	...
	public function getColumn ($pColumn) {
		...
		if (!isset($this->_columns[$pColumn])) {
			$this->_columns[$pColumn] = new PHPExcel_Worksheet_AutoFilter_Column($pColumn,
																				 $this);
		}
	 ...
	 * @return PHPExcel_Worksheet_AutoFilter
	 */
	public function shiftColumn ($fromColumn = NULL,
								 $toColumn = NULL) {
				...
				case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER :
					...
					foreach ($rules as $rule) {
						...
						if (($dynamicRuleType == PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE) ||
							...
							$average = PHPExcel_Calculation::getInstance ()->calculateFormula ($averageFormula,
																							   NULL,
...
//			echo (($result) ? 'SHOW' : 'HIDE'),PHP_EOL;
	 ...
	 * @return mixed[]
	 */
	private function _dynamicFilterDateRange ($dynamicRuleType,
											  &$filterColumn) {
		...
		switch ($dynamicRuleType) {
			...
			case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_TOMORROW :
				$maxVal = (int)PHPExcel_Shared_Date::PHPtoExcel (strtotime ('+1 day',
																			$baseDate));
			...
			case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_YEARTODATE :
				$maxVal = (int)PHPExcel_Shared_Date::PHPtoExcel (strtotime ('+1 day',
																			$baseDate));
				$val = (int)PHPExcel_Shared_Date::PHPToExcel (gmmktime (0,
																		0,
																		0,
																		1,
																		1,
	...
	private function _calculateTopTenValue ($columnID,
											$startRow,
											$endRow,
											$ruleType,
											$ruleValue) {
		...
		$dataValues = PHPExcel_Calculation_Functions::flattenArray (
			$this->_workSheet->rangeToArray ($range,
											 NULL,
											 TRUE,
											 FALSE)
		);
		...
		return array_pop (array_slice ($dataValues,
									   0,
									   $ruleValue));
	}

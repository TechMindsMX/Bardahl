...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	public function render ($outputDestination) {
		...
		if ($groupCount == 1) {
			...
			if (count ($chartTypes) == 1) {
				...
				return $this->_renderCombinationChart ($groupCount,
													   $dimensions,
													   $outputDestination);
			}
		}
		...
		switch ($chartType) {
			case 'area3DChart' :
				$dimensions = '3d';
			case 'areaChart' :
				$this->_renderAreaChart ($groupCount,
										 $dimensions);
				break;
			case 'bar3DChart' :
				$dimensions = '3d';
			case 'barChart' :
				$this->_renderBarChart ($groupCount,
										$dimensions);
				break;
			case 'line3DChart' :
				$dimensions = '3d';
			case 'lineChart' :
				$this->_renderLineChart ($groupCount,
										 $dimensions);
				break;
			case 'pie3DChart' :
				$dimensions = '3d';
			case 'pieChart' :
				$this->_renderPieChart ($groupCount,
										$dimensions,
										False,
										False);
				break;
			...
			case 'surface3DChart' :
				$dimensions = '3d';
			case 'surfaceChart' :
				$this->_renderContourChart ($groupCount,
											$dimensions);
				break;
			case 'stockChart' :
	...
	private function _renderCombinationChart ($groupCount,
											  $dimensions,
											  $outputDestination) {
	...
	private function _renderCartesianPlotArea ($type = 'textlin') {
		$this->_graph = new Graph(self::$_width,
								  self::$_height);
		...
		if (!is_null ($xAxisLabel)) {
			...
			if (!is_null ($title)) {
				$this->_graph->xaxis->SetTitle ($title,
												'center');
		...
		if (!is_null ($yAxisLabel)) {
			...
			if (!is_null ($title)) {
				$this->_graph->yaxis->SetTitle ($title,
												'center');
				if ($reverse) {
	...
	private function _renderPlotLine ($groupID,
		...
		for ($i = 0; $i < $seriesCount; ++$i) {
			...
			if ($filled) {
				...
				$this->_formatPointMarker ($seriesPlot,
										   $marker);
			}
	...
	private function _formatDataSetLabels ($groupID,
										   $datasetLabels,
										   $labelCount,
										   $rotation = '') {
	...
	private function _percentageSumCalculation ($groupID,
												$seriesCount) {
	...
	private function _percentageAdjustValues ($dataValues,
											  $sumValues) {
		foreach ($dataValues as $k => $dataValue) {
	...
	private function _formatPointMarker ($seriesPlot,
										 $markerID) {
	...
	private function _renderPlotBar ($groupID,
									 $dimensions = '2d') {
		...
		if ($labelCount > 0) {
			...
			if ($rotation == 'bar') {
				...
				$this->_graph->yaxis->SetLabelAlign ('center',
													 'top');
	...
	private function _renderPlotScatter ($groupID,
										 $bubble) {
		$grouping = $this->_chart->getPlotArea ()->getPlotGroupByIndex ($groupID)->getPlotGrouping ();
	...
	private function _renderLegend () {
		...
		if (!is_null ($legend)) {
			...
			switch ($legendPosition) {
				case 'r'    :
					$this->_graph->legend->SetPos (0.01,
												   0.5,
												   'right',
												   'center');    //	right
	...
	private function _renderAreaChart ($groupCount,
									   $dimensions = '2d') {
		require_once ('jpgraph_line.php');
	...
	private function _renderBarChart ($groupCount,
									  $dimensions = '2d') {
		require_once ('jpgraph_bar.php');
	...
	private function _renderLineChart ($groupCount,
									   $dimensions = '2d') {
		require_once ('jpgraph_line.php');
	...
	private function _renderPieChart ($groupCount,
									  $dimensions = '2d',
									  $doughnut = False,
									  $multiplePlots = False) {
	...
	private function _renderPiePlotArea ($doughnut = False) {
		$this->_graph = new PieGraph(self::$_width,
									 self::$_height);
	...
	private function _renderRadarPlotArea () {
		$this->_graph = new RadarGraph(self::$_width,
									   self::$_height);
	...
	private function _renderContourChart ($groupCount,
										  $dimensions) {

...
class PHPExcel_CachedObjectStorage_Memcache extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache
{
	...
	public function __construct (PHPExcel_Worksheet $parent,
		...
		if (is_null ($this->_cachePrefix)) {
			...
			$this->_cachePrefix = substr (md5 ($baseUnique),
										  0,
										  8) . '.';
	 ...
	 */
	public function addCacheData ($pCoord,
								  PHPExcel_Cell $cell) {
	...
	protected function _storeData () {
		if ($this->_currentCellIsDirty && !empty($this->_currentObjectID)) {
			...
			if (!$this->_memcache->replace ($this->_cachePrefix . $this->_currentObjectID . '.cache',
											$obj,
											NULL,
											$this->_cacheTime)
			) {
				if (!$this->_memcache->add ($this->_cachePrefix . $this->_currentObjectID . '.cache',
											$obj,
											NULL,
											$this->_cacheTime)
				) {
	...
	public function copyCellCollection (PHPExcel_Worksheet $parent) {
		...
		$baseUnique = $this->_getUniqueID ();
		$newCachePrefix = substr (md5 ($baseUnique),
								  0,
								  8) . '.';
		$cacheList = $this->getCellList ();
		foreach ($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if (!$this->_memcache->add ($newCachePrefix . $cellID . '.cache',
											$obj,
											NULL,
											$this->_cacheTime)
				) {
	 ...
	 * @throws    PHPExcel_Exception
	 */
	public function failureCallback ($host,
									 $port) {

...
class PHPExcel_Calculation
{
		...
		if (in_array ($language,
			...
			if ($locale != 'en_us') {
				...
				$functionNamesFile = PHPEXCEL_ROOT . 'PHPExcel' . DIRECTORY_SEPARATOR . 'locale' . DIRECTORY_SEPARATOR . str_replace ('_',
																																	  DIRECTORY_SEPARATOR,
																																	  $locale) . DIRECTORY_SEPARATOR . 'functions';
				...
				foreach ($localeFunctions as $localeFunction) {
					...
					if (strpos ($localeFunction,
								'=') !== FALSE
					) {
						list($fName, $lfName) = explode ('=',
														 $localeFunction);
						$fName = trim ($fName);
						$lfName = trim ($lfName);
				...
				$configFile = PHPEXCEL_ROOT . 'PHPExcel' . DIRECTORY_SEPARATOR . 'locale' . DIRECTORY_SEPARATOR . str_replace ('_',
																															   DIRECTORY_SEPARATOR,
																															   $locale) . DIRECTORY_SEPARATOR . 'config';
	...
	private static function _loadLocales () {
		...
		foreach (glob ($localeFileDirectory . '/*',
					   GLOB_ONLYDIR) as $filename) {
			$filename = substr ($filename,
								strlen ($localeFileDirectory) + 1);
			if ($filename != 'en') {
	...
	private static function _translateFormula ($from,
											   $to,
											   $formula,
											   $fromSeparator,
											   $toSeparator) {
	...
	public static function _translateSeparator ($fromSeparator,
												$toSeparator,
												$formula,
												&$inBraces) {
		$strlen = mb_strlen ($formula);
		for ($i = 0; $i < $strlen; ++$i) {
			$chr = mb_substr ($formula,
							  $i,
							  1);
			switch ($chr) {
				case '{' :
	 ...
	 * @throws    PHPExcel_Calculation_Exception
	 */
	public function calculateCellValue (PHPExcel_Cell $pCell = NULL,
										$resetLog = TRUE) {
		if ($pCell === NULL) {
			return NULL;
		}
		...
		try {
			$result = self::_unwrapResult ($this->_calculateFormulaValue ($pCell->getValue (),
																		  $pCell->getCoordinate (),
																		  $pCell));
		}
		catch (PHPExcel_Exception $e) {
	...
	public static function _unwrapResult ($value) {
		if (is_string ($value)) {
			if ((isset($value{0})) && ($value{0} == '"') && (substr ($value,
																	 -1) == '"')
			) {
				return substr ($value,
							   1,
	 ...
	 * @throws    PHPExcel_Calculation_Exception
	 */
	public function _calculateFormulaValue ($formula,
											$cellID = null,
		...
		if (($cellID !== NULL) && ($this->getValueFromCache ($wsTitle,
															 $cellID,
															 $cellValue))
		) {
			return $cellValue;
	...
	public function getValueFromCache ($worksheetName,
									   $cellID,
									   &$cellValue) {
...
//echo 'Retrieve from cache',PHP_EOL;
	...
	private function _processTokenStack ($tokens,
										 $cellID = NULL,
										 PHPExcel_Cell $pCell = NULL) {
		if ($tokens == FALSE) {
			return FALSE;
...
//				echo 'Token is a binary operator<br />';
				...
				switch ($token) {
					...
					case ':'    :            //	Range
						...
						if (strpos ($operand2Data['reference'],
									'!') !== FALSE
						) {
					...
					case '&'    :            //	Concatenation
						...
						if ((is_array ($operand1)) || (is_array ($operand2))) {
							...
							self::_checkMatrixOperands ($operand1,
														$operand2,
														2);
							try {
...
//					echo 'Token is a percentile operator<br />';
			...
			} elseif (preg_match ('/^' . self::CALCULATION_REGEXP_CELLREF . '$/i',
								  $token,
								  $matches)) {
				$cellRef = NULL;
...
//							echo '$cellRef='.$cellRef.' in current worksheet<br />';
			...
			} elseif (preg_match ('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i',
								  $token,
								  $matches)) {
...
//					} else {
					...
					if (strpos ($functionCall,
						...
						unset($arg);
						$result = call_user_func_array ($functionCall,
														$args);
					}
//					}
					if ($functionName != 'MKMATRIX') {
...
//					echo 'Token is a number, boolean, string, null or an Excel error<br />';
				...
				} elseif (preg_match ('/^' . self::CALCULATION_REGEXP_NAMEDRANGE . '$/i',
									  $token,
									  $matches)) {
...
//		}
				...
				foreach ($value as $row) {
					if (is_array ($row)) {
						$returnMatrix[] = implode ($pad,
												   array_map (array ($this, '_showValue'),
															  $row));
						$rpad = '; ';
					} else {
	...
	private function _executeBinaryComparisonOperation ($cellID,
														$operand1,
														$operand2,
														$operation,
														&$stack,
	 ...
	 *                                            2 = extend to fit
	 */
	private static function _checkMatrixOperands (&$operand1,
												  &$operand2,
												  $resize = 1) {
		...
		if ($resize == 2) {
			...
			self::_resizeMatricesExtend ($operand1,
										 $operand2,
										 $matrix1Rows,
										 $matrix1Columns,
										 $matrix2Rows,
			...
			self::_resizeMatricesShrink ($operand1,
										 $operand2,
										 $matrix1Rows,
										 $matrix1Columns,
										 $matrix2Rows,
	...
	public static function _getMatrixDimensions (&$matrix) {
		...
		foreach ($matrix as $rowKey => $rowValue) {
			$matrixColumns = max (count ($rowValue),
								  $matrixColumns);
	 ...
	 */
	private static function _resizeMatricesExtend (&$matrix1,
												   &$matrix2,
												   $matrix1Rows,
												   $matrix1Columns,
												   $matrix2Rows,
	 ...
	 * @param    integer $matrix2Columns Column size of second matrix operand
	 */
	private static function _resizeMatricesShrink (&$matrix1,
												   &$matrix2,
												   $matrix1Rows,
												   $matrix1Columns,
												   $matrix2Rows,
	 ...
	 * @return integer
	 */
	private function strcmpLowercaseFirst ($str1,
										   $str2) {
		$from = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
		$to = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		$inversedStr1 = strtr ($str1,
							   $from,
							   $to);
		$inversedStr2 = strtr ($str2,
							   $from,
							   $to);

		return strcmp ($inversedStr1,
					   $inversedStr2);
	}
...
//				echo 'Adjusted Range reference is '.$pRange,PHP_EOL;
	...
	private function _executeNumericBinaryOperation ($cellID,
													 $operand1,
													 $operand2,
													 $operation,
													 $matrixFunction,
		...
		if (!$this->_validateBinaryOperand ($cellID,
											$operand1,
											$stack)
		) {
			return FALSE;
	...
	private function _validateBinaryOperand ($cellID,
											 &$operand,
											 &$stack) {
		if (is_array ($operand)) {
			if ((count ($operand,
						COUNT_RECURSIVE) - count ($operand)) == 1
			) {
				do {
...
//				echo 'Adjusted Range reference is '.$pRange,PHP_EOL;
			...
			// Named range?
			$namedRange = PHPExcel_NamedRange::resolveRange ($pRange,
															 $pSheet);
			if ($namedRange !== NULL) {
...
//					echo 'Element '.$val.' is a Function<br />';
					if (isset(self::$_PHPExcelFunctions[strtoupper ($matches[1])]) || isset(self::$_controlFunctions[strtoupper ($matches[1])])) {    // it's a function
						...
						$ax = preg_match ('/^\s*(\s*\))/i',
										  ...
										  $amatch);
						if ($ax) {
							$stack->push ('Operand Count for Function ' . strtoupper ($val) . ')',
										  0);
							$expectingOperator = TRUE;
						} else {
							$stack->push ('Operand Count for Function ' . strtoupper ($val) . ')',
...
//					echo 'Element '.$val.' is a Cell reference<br />';
					...
					if ($testPrevOp['value'] == ':') {
						...
						if ($matches[2] == '') {
							...
							preg_match ('/^' . self::CALCULATION_REGEXP_CELLREF . '$/i',
										$startCellRef,
										$startMatches);
							if ($startMatches[2] > '') {
...
//					echo 'Element is an Intersect Operator<br />';
	...
	public function saveValueToCache ($worksheetName,
									  $cellID,
									  $cellValue) {
	...
	public function calculateFormula ($formula,
		...
		try {
			$result = self::_unwrapResult ($this->_calculateFormulaValue ($formula,
																		  $cellID,
																		  $pCell));
		}
		catch (PHPExcel_Exception $e) {

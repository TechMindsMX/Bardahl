...
class PHPExcel_Style_Color extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
	 ...
	 */
	public static function changeBrightness ($hex,
											 $adjustPercentage) {
		$rgba = (strlen ($hex) == 8);

		$red = self::getRed ($hex,
							 FALSE);
		$green = self::getGreen ($hex,
								 FALSE);
		$blue = self::getBlue ($hex,
							   FALSE);
		...
		$rgb = strtoupper (str_pad (dechex ($red),
									2,
									'0',
									0) .
						   str_pad (dechex ($green),
									2,
									'0',
									0) .
						   str_pad (dechex ($blue),
	 ...
	 */
	public static function getRed ($RGB,
								   $hex = TRUE) {
		return self::_getColourComponent ($RGB,
										  strlen ($RGB) - 6,
										  $hex);
	}
	 ...
	 */
	private static function _getColourComponent ($RGB,
												 $offset,
												 $hex = TRUE) {
		$colour = substr ($RGB,
						  $offset,
	 ...
	 */
	public static function getGreen ($RGB,
									 $hex = TRUE) {
		return self::_getColourComponent ($RGB,
										  strlen ($RGB) - 4,
										  $hex);
	 ...
	 */
	public static function getBlue ($RGB,
									$hex = TRUE) {
		return self::_getColourComponent ($RGB,
										  strlen ($RGB) - 2,
										  $hex);
	...
	public function applyFromArray ($pStyles = NULL) {
		if (is_array ($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if (array_key_exists ('rgb',
									  $pStyles)) {
					$this->setRGB ($pStyles['rgb']);
				}
				if (array_key_exists ('argb',
									  $pStyles)) {

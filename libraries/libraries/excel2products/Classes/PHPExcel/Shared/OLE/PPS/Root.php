...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
	 ...
	 */
	public function __construct ($time_1st,
								 $time_2nd,
								 $raChild) {
	...
	public function save ($filename) {
		...
		if (is_resource ($filename)) {
			...
			if ($filename == '-' || $filename == '') {
				...
				$this->_tmp_filename = tempnam ($this->_tmp_dir,
												"OLE_PPS_Root");
				$this->_FILEH_ = fopen ($this->_tmp_filename,
										"w+b");
				if ($this->_FILEH_ == false) {
					throw new PHPExcel_Writer_Exception("Can't create temporary file.");
				}
			} else {
				$this->_FILEH_ = fopen ($filename,
										"wb");
			}
		}
		if ($this->_FILEH_ == false) {
		...
		// Write BB
		$this->_saveBigData ($iSBDcnt,
							 $aList);
		// Write PPS
		$this->_savePps ($aList);
		// Write Big Block Depot and BDList and Adding Header informations
		$this->_saveBbd ($iSBDcnt,
						 $iBBcnt,
						 $iPPScnt);
	 ...
	 */
	public function _saveHeader ($iSBDcnt,
								 $iBBcnt,
								 $iPPScnt) {
		$FILE = $this->_FILEH_;
		...
		fwrite ($FILE,
				...
				. pack ("V",
						$iBdCnt)
				. pack ("V",
						$iBBcnt + $iSBDcnt) //ROOT START
				. pack ("V",
						0)
				. pack ("V",
						0x1000)
				. pack ("V",
						$iSBDcnt ? 0 : -2)                  //Small Block Depot
				. pack ("V",
						$iSBDcnt)
		);
		...
		if ($iBdCnt < $i1stBdL) {
			fwrite ($FILE,
					pack ("V",
						  -2)      // Extra BDList Start
					. pack ("V",
							0)        // Extra BDList Count
			);
		} else {
			fwrite ($FILE,
					pack ("V",
	 ...
	 * @param array &$raList Reference to array of PPS's
	 */
	public function _saveBigData ($iStBlk,
								  &$raList) {
		$FILE = $this->_FILEH_;
	...
	public function _savePps (&$raList) {
		...
		if ($iCnt % $iBCnt) {
			fwrite ($this->_FILEH_,
					str_repeat ("\x00",
								($iBCnt - ($iCnt % $iBCnt)) * PHPExcel_Shared_OLE::OLE_PPS_SIZE));
		}
	}
	 ...
	 * @param integer $iPpsCnt
	 */
	public function _saveBbd ($iSbdSize,
							  $iBsize,
							  $iPpsCnt) {
		$FILE = $this->_FILEH_;
		// Calculate Basic Setting

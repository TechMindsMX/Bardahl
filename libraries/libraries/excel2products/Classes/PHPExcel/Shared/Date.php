...
class PHPExcel_Shared_Date
{
	...
	public static function ExcelToPHP ($dateValue = 0,
		...
		if ($dateValue >= 1) {
			...
			$returnValue = (integer)gmmktime ($hours,
											  $mins,
											  $secs);
		}
	...
	public static function PHPToExcel ($dateValue = 0,
		...
		if ((is_object ($dateValue)) && ($dateValue instanceof DateTime)) {
			...
			$retValue = self::FormattedPHPToExcel (date ('Y',
														 $dateValue),
												   date ('m',
														 $dateValue),
												   date ('d',
	 ...
	 */
	public static function FormattedPHPToExcel ($year,
												$month,
												$day,
												$hours = 0,
												$minutes = 0,
	...
	public static function isDateTimeFormatCode ($pFormatCode = '') {
		...
		if (preg_match ('/(^|\])[^\[]*[' . self::$possibleDateFormatCharacters . ']/i',
						$pFormatCode)) {
			//	We might also have a format mask containing quoted strings...
			//		we don't want to test for any of our characters within the quoted blocks
			if (strpos ($pFormatCode,
						'"') !== FALSE
			) {
				$segMatcher = FALSE;
				foreach (explode ('"',
	...
	public static function dayStringToNumber ($day) {
		$strippedDayValue = (str_replace (self::$_numberSuffixes,
										  '',
										  $day));

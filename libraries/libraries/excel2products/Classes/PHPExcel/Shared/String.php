...
class PHPExcel_Shared_String
{
	...
	public static function getIsIconvEnabled () {
		...
		if (defined ('PHP_OS') && @stristr (PHP_OS,
											'AIX')
	...
	public static function UTF8toBIFF8UnicodeShort ($value,
		...
		if (empty($arrcRuns)) {
			...
			$data = pack ('CC',
						  ...
						  $opt);
			// characters
			$data .= self::ConvertEncoding ($value,
											'UTF-16LE',
											'UTF-8');
		} else {
			$data = pack ('vC',
	...
	public static function UTF8toBIFF8UnicodeLong ($value) {
		...
		// characters
		$chars = self::ConvertEncoding ($value,
										'UTF-16LE',
										'UTF-8');
	 ...
	 * @return string
	 */
	public static function Substring ($pValue = '',
									  $pStart = 0,
									  $pLength = 0) {
		if (self::getIsMbstringEnabled ()) {
			return mb_substr ($pValue,
							  $pStart,
							  $pLength,
							  'UTF-8');
		}
	...
	public static function convertToNumberIfFraction (&$operand) {
		if (preg_match ('/^' . self::STRING_REGEXP_FRACTION . '$/i',
						$operand,
						$match)) {
			$sign = ($match[1] == '-') ? '-' : '+';
	...
	public static function testStringAsNumeric ($value) {
		...
		$v = floatval ($value);
		return (is_numeric (substr ($value,
									0,
									strlen ($v)))) ? $v : $value;
	}
}

...
class PHPExcel_Polynomial_Best_Fit extends PHPExcel_Best_Fit
{
	...
	function __construct ($order,
						  ...
						  $xValues = array (),
						  $const = True) {
		if (parent::__construct ($yValues,
								 $xValues) !== False
		) {
			if ($order < $this->_valueCount) {
				$this->_bestFitType .= '_' . $order;
				$this->_order = $order;
				$this->_polynomial_regression ($order,
											   $yValues,
											   $xValues,
											   $const);
	 ...
	 * @param    boolean $const
	 */
	private function _polynomial_regression ($order,
											 $yValues,
											 $xValues,
											 $const) {
		// calculate sums
	...
	public function getSlope ($dp = 0) {
		if ($dp != 0) {
			...
			foreach ($this->_slope as $coefficient) {
				$coefficients[] = round ($coefficient,
										 $dp);
			}
			return $coefficients;
		}
	...
	public function getCoefficients ($dp = 0) {
		return array_merge (array ($this->getIntersect ($dp)),
							$this->getSlope ($dp));

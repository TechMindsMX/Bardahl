	...
	public static function calculate ($trendType = self::TREND_BEST_FIT,
		...
		switch ($trendType) {
			...
			case self::TREND_POLYNOMIAL_6    :
				if (!isset(self::$_trendCache[$key])) {
					$order = substr ($trendType,
									 -1);
					self::$_trendCache[$key] = new PHPExcel_Polynomial_Best_Fit($order,
																				$yValues,
																				$xValues,
																				$const);
				}
			...
			case self::TREND_BEST_FIT_NO_POLY    :
				...
				if ($trendType != self::TREND_BEST_FIT_NO_POLY) {
					foreach (self::$_trendTypePolyOrders as $trendMethod) {
						$order = substr ($trendMethod,
										 -1);
						$bestFit[$trendMethod] = new PHPExcel_Polynomial_Best_Fit($order,
																				  $yValues,
																				  $xValues,
																				  $const);

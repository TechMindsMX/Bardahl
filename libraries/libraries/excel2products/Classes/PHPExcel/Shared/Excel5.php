...
class PHPExcel_Shared_Excel5
{
	...
	public static function getDistanceX (PHPExcel_Worksheet $sheet,
		...
		$distanceX -= (int)floor (self::sizeCol ($sheet,
												 $startColumn) * $startOffsetX / 1024);

		// correct for offsetX in endcell
		$distanceX -= (int)floor (self::sizeCol ($sheet,
												 $endColumn) * (1 - $endOffsetX / 1024));
	 ...
	 */
	public static function sizeCol ($sheet,
									$col = 'A') {
		...
		if (isset($columnDimensions[$col]) and $columnDimensions[$col]->getWidth () != -1) {
			...
			if ($sheet->getDefaultColumnDimension ()->getWidth () != -1) {
				...
				$pixelWidth = PHPExcel_Shared_Font::getDefaultColumnWidthByFont ($font,
																				 true);
			}
		}
	 ...
	 */
	public static function getDistanceY (PHPExcel_Worksheet $sheet,
										 $startRow = 1,
										 $startOffsetY = 0,
										 $endRow = 1,
										 $endOffsetY = 0) {
		...
		for ($row = $startRow; $row <= $endRow; ++$row) {
			$distanceY += self::sizeRow ($sheet,
										 $row);
		}

		// correct for offsetX in startcell
		$distanceY -= (int)floor (self::sizeRow ($sheet,
												 $startRow) * $startOffsetY / 256);

		// correct for offsetX in endcell
		$distanceY -= (int)floor (self::sizeRow ($sheet,
												 $endRow) * (1 - $endOffsetY / 256));
	 ...
	 * @return integer The width in pixels
	 */
	public static function sizeRow ($sheet,
									$row = 1) {
		// default font of the workbook
		$font = $sheet->getParent ()->getDefaultStyle ()->getFont ();
	 ...
	 * @return array
	 */
	public static function oneAnchor2twoAnchor ($sheet,
												$coordinates,
												$offsetX,
												$offsetY,
												$width,
		...
		while ($height >= self::sizeRow ($sheet,
										 $row_end + 1)) {
			$height -= self::sizeRow ($sheet,
									  $row_end + 1);
			++$row_end;
		}

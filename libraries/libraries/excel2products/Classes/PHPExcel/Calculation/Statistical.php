...
class PHPExcel_Calculation_Statistical
{
	 ...
	 * @return    float
	 */
	public static function AVERAGEIF ($aArgs,
									  $condition,
									  $averageArgs = array ()) {
		// Return value
		$returnValue = 0;
	 ...
	 *
	 */
	public static function BETAINV ($probability,
									$alpha,
									$beta,
									$rMin = 0,
									$rMax = 1) {
		...
		if ((is_numeric ($probability)) && (is_numeric ($alpha)) && (is_numeric ($beta)) && (is_numeric ($rMin)) && (is_numeric ($rMax))) {
			...
			return round ($rMin + $guess * ($rMax - $rMin),
						  12);
		}
	 ...
	 *
	 */
	public static function BETADIST ($value,
									 $alpha,
									 $beta,
									 $rMin = 0,
									 $rMax = 1) {
		...
		if ((is_numeric ($value)) && (is_numeric ($alpha)) && (is_numeric ($beta)) && (is_numeric ($rMin)) && (is_numeric ($rMax))) {
			...
			$value -= $rMin;
			$value /= ($rMax - $rMin);
			return self::_incompleteBeta ($value,
										  $alpha,
										  $beta);
		}
		return PHPExcel_Calculation_Functions::VALUE ();
	 ...
	 */
	private static function _incompleteBeta ($x,
											 $p,
											 $q) {
		if ($x <= 0.0) {
			return 0.0;
		...
		$beta_gam = exp ((0 - self::_logBeta ($p,
											  $q)) + $p * log ($x) + $q * log (1.0 - $x));
		if ($x < ($p + 1.0) / ($p + $q + 2.0)) {
			return $beta_gam * self::_betaFraction ($x,
													$p,
													$q) / $p;
		} else {
			return 1.0 - ($beta_gam * self::_betaFraction (1 - $x,
														   $q,
														   $p) / $q);
		}
	}    //	function _logGamma()
	 ...
	 * @author Jaco van Kooten
	 */
	private static function _betaFraction ($x,
										   $p,
										   $q) {
		$c = 1.0;
		$sum_pq = $p + $q;
	 ...
	 *
	 */
	public static function BINOMDIST ($value,
									  $trials,
									  $probability,
									  $cumulative) {
			...
			if ((is_numeric ($cumulative)) || (is_bool ($cumulative))) {
				if ($cumulative) {
					...
					return PHPExcel_Calculation_MathTrig::COMBIN ($trials,
																  $value) * pow ($probability,
																				 $value) * pow (1 - $probability,
																								$trials - $value);
				}
			}
		}
	 ...
	 * @return    float
	 */
	public static function CHIINV ($probability,
								   $degrees) {
		$probability = PHPExcel_Calculation_Functions::flattenSingleValue ($probability);
	 ...
	 * @return    float
	 */
	public static function CHIDIST ($value,
									$degrees) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		$degrees = floor (PHPExcel_Calculation_Functions::flattenSingleValue ($degrees));
		...
		if ((is_numeric ($value)) && (is_numeric ($degrees))) {
			...
			return 1 - (self::_incompleteGamma ($degrees / 2,
												$value / 2) / self::_gamma ($degrees / 2));
		}
		return PHPExcel_Calculation_Functions::VALUE ();
	}    //	function _inverse_ncdf2()

	private static function _incompleteGamma ($a,
											  $x) {
		static $max = 32;
		$summer = 0;
		for ($n = 0; $n <= $max; ++$n) {
	 ...
	 *
	 */
	public static function CONFIDENCE ($alpha,
									   $stdDev,
									   $size) {
		$alpha = PHPExcel_Calculation_Functions::flattenSingleValue ($alpha);
	 ...
	 *
	 */
	public static function NORMINV ($probability,
									$mean,
									$stdDev) {
		$probability = PHPExcel_Calculation_Functions::flattenSingleValue ($probability);
	 ...
	 * @return    float
	 */
	public static function CORREL ($yValues,
								   $xValues = null) {
		if ((is_null ($xValues)) || (!is_array ($yValues)) || (!is_array ($xValues))) {
			return PHPExcel_Calculation_Functions::VALUE ();
		}
		if (!self::_checkTrendArrays ($yValues,
									  $xValues)
		) {
			return PHPExcel_Calculation_Functions::VALUE ();
		}
	...
	private static function _checkTrendArrays (&$array1,
											   &$array2) {
		if (!is_array ($array1)) {
			$array1 = array ($array1);
		}
	 ...
	 * @return    int
	 */
	public static function COUNTIF ($aArgs,
									$condition) {
		// Return value
		$returnValue = 0;
	 ...
	 * @return    float
	 */
	public static function COVAR ($yValues,
								  $xValues) {
		if (!self::_checkTrendArrays ($yValues,
									  $xValues)
		) {
	 ...
	 *
	 */
	public static function CRITBINOM ($trials,
									  $probability,
									  $alpha) {
		$trials = floor (PHPExcel_Calculation_Functions::flattenSingleValue ($trials));
		$probability = PHPExcel_Calculation_Functions::flattenSingleValue ($probability);
...
//			$CumPGuessMinus1 = $CumPGuess - $PGuess;
	 ...
	 * @return    float
	 */
	public static function EXPONDIST ($value,
									  $lambda,
									  $cumulative) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
	 ...
	 * @return    float
	 */
	public static function FORECAST ($xValue,
									 $yValues,
									 $xValues) {
	 ...
	 *
	 */
	public static function GAMMAINV ($probability,
									 $alpha,
									 $beta) {
		$probability = PHPExcel_Calculation_Functions::flattenSingleValue ($probability);
		$alpha = PHPExcel_Calculation_Functions::flattenSingleValue ($alpha);
	 ...
	 *
	 */
	public static function GAMMADIST ($value,
									  $a,
									  $b,
									  $cumulative) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		...
		if ((is_numeric ($value)) && (is_numeric ($a)) && (is_numeric ($b))) {
			...
			if ((is_numeric ($cumulative)) || (is_bool ($cumulative))) {
				if ($cumulative) {
					return self::_incompleteGamma ($a,
												   $value / $b) / self::_gamma ($a);
				} else {
					return (1 / (pow ($b,
									  $a) * self::_gamma ($a))) * pow ($value,
																	   $a - 1) * exp (0 - ($value / $b));
				}
			}
		}
	 ...
	 */
	public static function GROWTH ($yValues,
								   $xValues = array (),
								   $newValues = array (),
								   $const = True) {
	 ...
	 *
	 */
	public static function HYPGEOMDIST ($sampleSuccesses,
										$sampleNumber,
										$populationSuccesses,
										$populationNumber) {
	 ...
	 * @return    float
	 */
	public static function INTERCEPT ($yValues,
									  $xValues) {
		if (!self::_checkTrendArrays ($yValues,
									  $xValues)
		) {
	 ...
	 * @return    array
	 */
	public static function LINEST ($yValues,
								   $xValues = NULL,
								   $const = TRUE,
								   $stats = FALSE) {
		$const = (is_null ($const)) ? TRUE : (boolean)PHPExcel_Calculation_Functions::flattenSingleValue ($const);
	 ...
	 * @return    array
	 */
	public static function LOGEST ($yValues,
								   $xValues = null,
								   $const = True,
								   $stats = False) {
		$const = (is_null ($const)) ? True : (boolean)PHPExcel_Calculation_Functions::flattenSingleValue ($const);
	 ...
	 */
	public static function LOGINV ($probability,
								   $mean,
								   $stdDev) {
		$probability = PHPExcel_Calculation_Functions::flattenSingleValue ($probability);
		$mean = PHPExcel_Calculation_Functions::flattenSingleValue ($mean);
	 ...
	 * @return    float
	 */
	public static function LOGNORMDIST ($value,
										$mean,
										$stdDev) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		$mean = PHPExcel_Calculation_Functions::flattenSingleValue ($mean);
	 ...
	 *
	 */
	public static function NORMDIST ($value,
									 $mean,
									 $stdDev,
									 $cumulative) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
	 ...
	 * @return    float
	 */
	public static function MAXIF ($aArgs,
								  $condition,
								  $sumArgs = array ()) {
		// Return value
		$returnValue = null;
	 ...
	 * @return    float
	 */
	public static function MINIF ($aArgs,
								  $condition,
								  $sumArgs = array ()) {
		// Return value
		$returnValue = null;
	 ...
	 *
	 */
	public static function NEGBINOMDIST ($failures,
										 $successes,
										 $probability) {
		...
		if ((is_numeric ($failures)) && (is_numeric ($successes)) && (is_numeric ($probability))) {
			...
			return (PHPExcel_Calculation_MathTrig::COMBIN ($failures + $successes - 1,
														   $successes - 1)) * (pow ($probability,
																					$successes)) * (pow (1 - $probability,
																										 $failures));
		}
		return PHPExcel_Calculation_Functions::VALUE ();
	}    //	function MODE()
	 ...
	 * @return    float
	 */
	public static function PERCENTRANK ($valueSet,
										$value,
										$significance = 3) {
		...
		return round ($pos / $valueAdjustor,
					  $significance);
	 ...
	 *
	 */
	public static function POISSON ($value,
									$mean,
									$cumulative) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		$mean = PHPExcel_Calculation_Functions::flattenSingleValue ($mean);
			...
			if ((is_numeric ($cumulative)) || (is_bool ($cumulative))) {
				if ($cumulative) {
					...
					for ($i = 0; $i <= floor ($value); ++$i) {
						$summer += pow ($mean,
										$i) / PHPExcel_Calculation_MathTrig::FACT ($i);
					}
					return exp (0 - $mean) * $summer;
				} else {
					return (exp (0 - $mean) * pow ($mean,
												   $value)) / PHPExcel_Calculation_MathTrig::FACT ($value);
				}
			}
		}
		return PHPExcel_Calculation_Functions::VALUE ();
	 ...
	 * @return    float
	 */
	public static function RSQ ($yValues,
								$xValues) {
		if (!self::_checkTrendArrays ($yValues,
									  $xValues)
		) {
	 ...
	 * @return    float
	 */
	public static function SLOPE ($yValues,
								  $xValues) {
		if (!self::_checkTrendArrays ($yValues,
									  $xValues)
		) {
	 ...
	 * @return    float
	 */
	public static function TDIST ($value,
								  $degrees,
								  $tails) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		$degrees = floor (PHPExcel_Calculation_Functions::flattenSingleValue ($degrees));
	 ...
	 * @return    array of float
	 */
	public static function TREND ($yValues,
								  $xValues = array (),
								  $newValues = array (),
								  $const = True) {
		$yValues = PHPExcel_Calculation_Functions::flattenArray ($yValues);
	 ...
	 *
	 */
	public static function WEIBULL ($value,
									$alpha,
									$beta,
									$cumulative) {
		$value = PHPExcel_Calculation_Functions::flattenSingleValue ($value);
		...
		if ((is_numeric ($value)) && (is_numeric ($alpha)) && (is_numeric ($beta))) {
			...
			if ((is_numeric ($cumulative)) || (is_bool ($cumulative))) {
				if ($cumulative) {
					return 1 - exp (0 - pow ($value / $beta,
											 $alpha));
				} else {
					return ($alpha / pow ($beta,
										  $alpha)) * pow ($value,
														  $alpha - 1) * exp (0 - pow ($value / $beta,
																					  $alpha));
				}
			}
		}
	 ...
	 *
	 */
	public static function ZTEST ($dataSet,
								  $m0,
								  $sigma = NULL) {
	 ...
	 */
	private static function _beta ($p,
								   $q) {
		if ($p <= 0.0 || $q <= 0.0 || ($p + $q) > LOG_GAMMA_X_MAX_VALUE) {
			return 0.0;
		} else {
			return exp (self::_logBeta ($p,
										$q));
		}
	}    //	function VARA()
	 ...
	 * @author Jaco van Kooten
	 */
	private static function _logBeta ($p,
									  $q) {

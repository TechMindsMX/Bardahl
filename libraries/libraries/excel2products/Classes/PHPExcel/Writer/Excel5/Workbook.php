...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
	 ...
	 */
	public function addXfWriter ($style,
								 $isStyleXf = false) {
	 ...
	 */
	private function _writeNumFormat ($format,
									  $ifmt) {
			...
			foreach ($namedRanges as $namedRange) {
				...
				try {
					...
					if (isset($formulaData{0}) and ($formulaData{0} == "\x7A" or $formulaData{0} == "\x5A")) {
						$formulaData = "\x3A" . substr ($formulaData,
														1);
					}
	 ...
	 */
	private function _writeDefinedNameBiff8 ($name,
											 $formulaData,
											 $sheetIndex = 0,
											 $isBuiltIn = false) {
		$record = 0x0018;
	 ...
	 * @return    string    Complete binary record data
	 * */
	private function _writeShortNameBiff8 ($name,
										   $sheetIndex = 0,
										   $rangeBounds,
										   $isHidden = false) {
		$record = 0x0018;
	...
	private function _writeSharedStringsTable () {
		...
		foreach (array_keys ($this->_str_table) as $string) {
			...
			$headerinfo = unpack ("vlength/Cencoding",
								  $string);
	 ...
	 */
	private function _writeBoundsheet ($sheet,
									   $offset) {
	 ...
	 */
	private function _writeNameShort ($index,
									  $type,
									  $rowmin,
									  $rowmax,
									  $colmin,
	 ...
	 * @param integer $colmax End column
	 */
	private function _writeNameLong ($index,
									 $type,
									 $rowmin,
									 $rowmax,
									 $colmin,

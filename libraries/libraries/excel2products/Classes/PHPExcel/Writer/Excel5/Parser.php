...
class PHPExcel_Writer_Excel5_Parser
{
	...
	function _packExtRef ($ext_ref) {
		...
		if (preg_match ("/:/",
						$ext_ref)) {
			list($sheet_name1, $sheet_name2) = explode (':',
														$ext_ref);
		...
		switch ($token) {
			...
			default:
				...
				elseif (preg_match ("/\"([^\"]|\"\"){0,255}\"/",
									$token) and $this->_lookahead != '"' and (substr_count ($token,
																							'"') % 2 == 0)
				) {
					return $token;
	...
	function _condition () {
		...
		if ($this->_current_token == "<") {
			...
			$result = $this->_createTree ('ptgLT',
										  $result,
										  $result2);
		} elseif ($this->_current_token == ">") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgGT',
										  $result,
										  $result2);
		} elseif ($this->_current_token == "<=") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgLE',
										  $result,
										  $result2);
		} elseif ($this->_current_token == ">=") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgGE',
										  $result,
										  $result2);
		} elseif ($this->_current_token == "=") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgEQ',
										  $result,
										  $result2);
		} elseif ($this->_current_token == "<>") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgNE',
										  $result,
										  $result2);
		} elseif ($this->_current_token == "&") {
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgConcat',
										  $result,
										  $result2);
		}
		return $result;
	...
	function _expression () {
		...
		if (preg_match ("/\"([^\"]|\"\"){0,255}\"/",
			...
			// If it's an error code
							  ...
							  $this->_current_token) or $this->_current_token == '#N/A'
		) {
			...
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgUminus',
										  $result2,
										  '');
			return $result;
			...
			$this->_advance ();
			$result2 = $this->_expression ();
			$result = $this->_createTree ('ptgUplus',
										  $result2,
										  '');
			return $result;
		}
		...
		while (($this->_current_token == "+") or
			...
			if ($this->_current_token == "+") {
				$this->_advance ();
				$result2 = $this->_term ();
				$result = $this->_createTree ('ptgAdd',
											  $result,
											  $result2);
			} elseif ($this->_current_token == "-") {
				$this->_advance ();
				$result2 = $this->_term ();
				$result = $this->_createTree ('ptgSub',
											  $result,
											  $result2);
			} else {
				$this->_advance ();
				$result2 = $this->_term ();
				$result = $this->_createTree ('ptgPower',
											  $result,
											  $result2);
			}
		}
	...
	function _term () {
		...
		while (($this->_current_token == "*") or
			...
			if ($this->_current_token == "*") {
				$this->_advance ();
				$result2 = $this->_fact ();
				$result = $this->_createTree ('ptgMul',
											  $result,
											  $result2);
			} else {
				$this->_advance ();
				$result2 = $this->_fact ();
				$result = $this->_createTree ('ptgDiv',
											  $result,
											  $result2);
			}
		}
	...
	function _parenthesizedExpression () {
		$result = $this->_createTree ('ptgParen',
									  $this->_expression (),
									  '');
		return $result;
	}
	...
	function _func () {
		...
		$result = $this->_createTree ($function,
									  $result,
									  $num_args);
		$this->_advance ();         // eat the ")"
		return $result;
	...
	function _convert ($token) {
		...
		} elseif (preg_match ('/^\$?([A-Ia-i]?[A-Za-z])\$?(\d+)$/',
							  $token)) {
	...
	function _convertString ($string) {
		...
		return pack ('C',
					 $this->ptg['ptgStr']) . PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort ($string);
	}
...
//		}
	 ...
	 */
	function _convertFunction ($token,
							   $num_args) {

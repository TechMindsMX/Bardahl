...
class PHPExcel_Writer_Excel2007_Workbook extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	public function writeWorkbook (PHPExcel $pPHPExcel = null,
		...
		if ($this->getParentWriter ()->getOffice2003Compatibility () === false) {
			$this->_writeBookViews ($objWriter,
									$pPHPExcel);
		}
		...
		// definedNames
		$this->_writeDefinedNames ($objWriter,
								   $pPHPExcel);
	...
	private function _writeFileVersion (PHPExcel_Shared_XMLWriter $objWriter = null) {
		...
		$objWriter->writeAttribute ('appName',
									'xl');
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeWorkbookProtection (PHPExcel_Shared_XMLWriter $objWriter = null,
	...
	private function _writeSheet (PHPExcel_Shared_XMLWriter $objWriter = null,
		...
		if ($pSheetname != '') {
			...
			$objWriter->writeAttribute ('name',
										$pSheetname);
			$objWriter->writeAttribute ('sheetId',
										$pSheetId);
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeDefinedNames (PHPExcel_Shared_XMLWriter $objWriter = null,
										 PHPExcel $pPHPExcel = null) {
		// Write defined names
		...
		if (count ($pPHPExcel->getNamedRanges ()) > 0) {
			// Named ranges
			$this->_writeNamedRanges ($objWriter,
									  $pPHPExcel);
		}
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeNamedRanges (PHPExcel_Shared_XMLWriter $objWriter = null,
										PHPExcel $pPHPExcel) {
		// Loop named ranges
		$namedRanges = $pPHPExcel->getNamedRanges ();
		foreach ($namedRanges as $namedRange) {
			$this->_writeDefinedNameForNamedRange ($objWriter,
												   $namedRange);
		}
	}
	...
	private function _writeDefinedNameForPrintArea (PHPExcel_Shared_XMLWriter $objWriter = null,
		...
		if ($pSheet->getPageSetup ()->isPrintAreaSet ()) {
			...
			$objWriter->writeRawData (implode (',',
											   $chunks));

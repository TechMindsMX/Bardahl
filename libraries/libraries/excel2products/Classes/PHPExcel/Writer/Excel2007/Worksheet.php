...
class PHPExcel_Writer_Excel2007_Worksheet extends PHPExcel_Writer_Excel2007_WriterPart
{
	 ...
	 */
	public function writeWorksheet ($pSheet = null,
									$pStringTable = null,
									$includeCharts = FALSE) {
		if (!is_null ($pSheet)) {
			// Create XML writer
			...
			// sheetPr
			$this->_writeSheetPr ($objWriter,
								  $pSheet);

			// Dimension
			$this->_writeDimension ($objWriter,
									$pSheet);
			...
			// cols
			$this->_writeCols ($objWriter,
							   $pSheet);

			// sheetData
			$this->_writeSheetData ($objWriter,
									$pSheet,
									$pStringTable);
			...
			// protectedRanges
			$this->_writeProtectedRanges ($objWriter,
										  $pSheet);

			// autoFilter
			$this->_writeAutoFilter ($objWriter,
									 $pSheet);

			// mergeCells
			$this->_writeMergeCells ($objWriter,
									 $pSheet);

			// conditionalFormatting
			$this->_writeConditionalFormatting ($objWriter,
												$pSheet);

			// dataValidations
			$this->_writeDataValidations ($objWriter,
										  $pSheet);

			// hyperlinks
			$this->_writeHyperlinks ($objWriter,
									 $pSheet);
			...
			// Page margins
			$this->_writePageMargins ($objWriter,
									  $pSheet);

			// Page setup
			$this->_writePageSetup ($objWriter,
									$pSheet);

			// Header / footer
			$this->_writeHeaderFooter ($objWriter,
									   $pSheet);

			// Breaks
			$this->_writeBreaks ($objWriter,
								 $pSheet);

			// Drawings and/or Charts
			$this->_writeDrawings ($objWriter,
								   $pSheet,
								   $includeCharts);

			// LegacyDrawing
			$this->_writeLegacyDrawing ($objWriter,
										$pSheet);

			// LegacyDrawingHF
			$this->_writeLegacyDrawingHF ($objWriter,
										  $pSheet);
	...
	private function _writeSheetPr (PHPExcel_Shared_XMLWriter $objWriter = null,
		...
		if (!empty($autoFilterRange)) {
			$objWriter->writeAttribute ('filterMode',
										1);
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeSheetViews (PHPExcel_Shared_XMLWriter $objWriter = NULL,
									   PHPExcel_Worksheet $pSheet = NULL) {
		// sheetViews
		...
		if (($topLeftCell != '') && ($topLeftCell != 'A1')) {
			...
			if ($xSplit > 1) {
				$objWriter->writeAttribute ('xSplit',
											$xSplit - 1);
			}
			if ($ySplit > 1) {
				$objWriter->writeAttribute ('ySplit',
											$ySplit - 1);
				$pane = ($xSplit > 1) ? 'bottomRight' : 'bottomLeft';
			}
			$objWriter->writeAttribute ('topLeftCell',
										$topLeftCell);
			$objWriter->writeAttribute ('activePane',
										$pane);
...
//				if ($pane != '') {
		...
		$objWriter->writeAttribute ('activeCell',
									$activeCell);
...
//				}
	...
	private function _writeSheetFormatPr (PHPExcel_Shared_XMLWriter $objWriter = null,
		...
		$objWriter->writeAttribute ('outlineLevelRow',
									(int)$outlineLevelRow);
		...
		$objWriter->writeAttribute ('outlineLevelCol',
									(int)$outlineLevelCol);
					...
					case 'f':            // Formula
						...
						if ($attributes['t'] == 'array') {
							...
							$objWriter->writeAttribute ('ca',
														'1');
							$objWriter->text (substr ($cellValue,
													  1));
							$objWriter->endElement ();
						} else {
...
//								$calculatedValue = $pCell->getCalculatedValue();
								if (!is_array ($calculatedValue) && substr ($calculatedValue,
																			0,
																			1) != '#'
								) {
									$objWriter->writeElement ('v',
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeAutoFilter (PHPExcel_Shared_XMLWriter $objWriter = null,
									   PHPExcel_Worksheet $pSheet = null) {
		$autoFilterRange = $pSheet->getAutoFilter ()->getRange ();
		if (!empty($autoFilterRange)) {
			// autoFilter
					...
					if (count ($rules > 0)) {
						...
						foreach ($rules as $rule) {
							if (($column->getFilterType () === PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER) &&
								...
								$objWriter->writeAttribute ('blank',
															1);
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeMergeCells (PHPExcel_Shared_XMLWriter $objWriter = null,
									   PHPExcel_Worksheet $pSheet = null) {
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writeHyperlinks (PHPExcel_Shared_XMLWriter $objWriter = null,
									   PHPExcel_Worksheet $pSheet = null) {
	 ...
	 * @throws    PHPExcel_Writer_Exception
	 */
	private function _writePageMargins (PHPExcel_Shared_XMLWriter $objWriter = null,
										PHPExcel_Worksheet $pSheet = null) {
		// pageMargins

...
var UniteSettingsBiz = new function() {
    ...
    this.getSettingsObject = function(formID) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            type = element.type;
            if (jQuery(element).attr("id") == "slide_text"){
            	
            	if(jQuery("#cke_slide_text").length == 1)
            		type = "ckeditor";
            	else
                    type = "editor";            	
            }            	            
            ...
            switch (type) {
                ...
                break;                
                case "ckeditor":
                	value = CKEDITOR.instances.slide_text.getData();
                break;
            ...
            switch (this.type) {
                ...
                case "disable":
                    ...
                    if (childInput) {		//disable
                        if (this.type == "enable" && controlValue != this.value || this.type == "disable" && controlValue == this.value) {
                            ...
                            if (jQuery(childInput).hasClass("inputColorPicker"))
                                g_picker.linkTo(childInput);
                        }
                ...
                case "show":
                    if (controlValue == this.value)
                        jQuery(childRow).show();
                    else
                        jQuery(childRow).hide();
                    break;
                case "hide":
                    if (controlValue == this.value)
                        jQuery(childRow).hide();

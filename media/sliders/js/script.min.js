...
$el.focus();}},};$('.nn_sliders > .accordion-group > .accordion-body').on('show show.bs.collapse',function(e){$(this).parent().addClass('active');e.stopPropagation();});$('.nn_sliders > .accordion-group > .accordion-body').on('hidden hidden.bs.collapse',function(e){$(this).parent().removeClass('active');e.stopPropagation();});if(nn_sliders_use_hash){if(window.location.hash){var id=window.location.hash.replace('#','');if(id.indexOf("&")==-1&&id.indexOf("=")==-1&&$('#'+decodeURIComponent(id)+'.accordion-body').length>0){if(!nn_sliders_urlscroll){$('html,body').animate({scrollTop:0});}
...
if(window.location.hash){var id=window.location.hash.replace('#','');var $el=$('a[name="'+id+'"][data-toggle!="collapse"]');if($el){var hasparents=0;$el.closest('.nn_sliders').closest('.accordion-body').each(function(i,parent){if(!$(parent).hasClass('in')){nnSliders.show(parent.id,0);hasparents=1;}});if(hasparents){setTimeout(function(){$('html,body').animate({scrollTop:$el.offset().top});},500);}}}$('.nn_sliders > .accordion-group > .accordion-body.in iframe').each(function(){$(this).attr('reloaded',true);});$('.nn_sliders > .accordion-group > .accordion-body').on('show show.bs.collapse',function($e){if(typeof initialize=='function'){initialize();}
$(this).find('iframe').each(function(){if(!$(this).attr('reloaded')){this.src+='';$(this).attr('reloaded',true);}});});$(window).resize(function(){if(typeof initialize=='function'){initialize();}
$('.nn_sliders > .accordion-group > .accordion-body iframe').each(function(){$(this).attr('reloaded',false);});$('.nn_sliders > .accordion-group > .accordion-body.in iframe').each(function(){this.src+='';$(this).attr('reloaded',true);});});}});})(jQuery);function openAllSliders(){(function($){$('.nn_sliders').each(function(e,el){id=$(el).find('.accordion-body').first().attr('id');nnSliders.show(id);});})(jQuery);}
